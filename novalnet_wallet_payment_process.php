...
if (isset($response['payment_name'])) {
	$_SESSION['payment'] = $response['payment_name'];
	$payment_modules = new payment($_SESSION['payment']);
} else {
	$payment_modules = new payment($_SESSION['payment']);
}

if (isset($_GET['status_code']) && $_GET['status_code'] == 100 && isset($_GET['tid'])) {
...
if (empty($_SESSION['nn_order']) || (!empty($_SESSION['nn_order']) && $_SESSION['nn_order']->content_type == 'virtual')) {
...
if (isset($response['customer_id']) && $response['customer_id'] == 0 && $gx_version >= '4.7.1.2') {
	if (isset($response['products_id']) && isset($response['products_qty'])) { // Add product to cart in product page
		$_SESSION['cart'] = new shoppingCart();
		$cart_object = $_SESSION['cart'];

		if (isset($response['products_id']) && is_numeric($response['products_id']) && isset($response['products_qty']) && $response['products_qty'] > 0) {
			$attribute_info = isset($response['attribute_info']) ? $response['attribute_info'] : null;
			$cart_quantity = (xtc_remove_non_numeric($response['products_qty']) + $cart_object->get_quantity(xtc_get_uprid($response['products_id'], $attribute_info)));
			$cart_object->add_cart((int)$response['products_id'], $cart_quantity, $attribute_info);
		}
	}
    $response = json_encode(['login_url' => $response['loginUrl'], 'isLogin'=> 1, 'shipping_page' => $response['shippingPage']], true);
    echo $response;
	exit;
...
if (!empty($received_address['order']['shipping']['contact']['countryCode']) || !empty($received_address['order']['billing']['contact']['countryCode'])) {
	$_SESSION['delivery_zone'] = (!empty($received_address['order']['shipping']['contact']['countryCode']) ? $received_address['order']['shipping']['contact']['countryCode'] : (!empty($received_address['order']['billing']['contact']['countryCode']) ? $received_address['order']['billing']['contact']['countryCode'] : $order->customer['country']['iso_code_2']));
}
...
if (!empty($received_address['order']['shipping']) && !empty($shipping_methods))
{
	foreach ($shipping_methods as $shipping_method) 
	{
		$shipping_id = explode('_', $received_address['order']['shipping']['method']['identifier']);
...
$_SESSION['shipping']['id']     = isset($received_address['order']['shipping']['method']['identifier']) ? $received_address['order']['shipping']['method']['identifier'] : '';
$_SESSION['received_data']      = $response['variable_name']['response'];
$_SESSION['token'] = $response['variable_name']['response']['transaction']['token'];
$order->info['shipping_method'] = isset($received_address['order']['shipping']['method']['label']) ? $received_address['order']['shipping']['method']['label'] : '';
$order->info['shipping_class']  = isset($received_address['order']['shipping']['method']['identifier']) ? $received_address['order']['shipping']['method']['identifier'] : '';
$order->info['payment_method']  = $_SESSION['payment'];
$order->info['payment_class']   = $_SESSION['payment'];
$shipping_cost = 0;

if (!empty($received_address['order']['shipping']) && !empty($received_address['order']['shipping']['method']['amount']))
{
	$shipping_cost = $received_address['order']['shipping']['method']['amount'] / 100;
}
$order->info['total']           = ($order->info['subtotal'] + $shipping_cost) - (round($discount_price, 2));

if (!empty($_SESSION['nn_order']->info['deduction'])) {
	$order->info['deduction'] = $_SESSION['nn_order']->info['deduction'];
}
...
$sql_data_array = array(
	...
	'customers_firstname' 			=> !empty($received_address['order']['shipping']['contact']['firstName']) ? $received_address['order']['shipping']['contact']['firstName'] : $received_address['order']['billing']['contact']['firstName'],
	'customers_lastname' 			=> !empty($received_address['order']['shipping']['contact']['lastName']) ? $received_address['order']['shipping']['contact']['lastName'] : $received_address['order']['billing']['contact']['lastName'],
	...
	'customers_country'             => isset($received_address['order']['billing']['contact']['country']) ? $received_address['order']['billing']['contact']['country'] : '',
	'customers_email_address' 		=> !empty($received_address['order']['billing']['contact']['email']) ? $received_address['order']['billing']['contact']['email'] : (!empty($received_address['order']['shipping']['contact']['email']) ? $received_address['order']['shipping']['contact']['email'] : $order->customer['email_address']),
	...
	'delivery_lastname' 			=> (empty($received_address['order']['shipping']['contact']['lastName']) ? $received_address['order']['billing']['contact']['lastName'] : $received_address['order']['shipping']['contact']['lastName']),
	...
	'shipping_method' 				=> !empty($received_address['order']['shipping']) ? $received_address['order']['shipping']['method']['label'] : null,
	'shipping_class' 				=> !empty($received_address['order']['shipping']) ? $received_address['order']['shipping']['method']['identifier'] : null,
