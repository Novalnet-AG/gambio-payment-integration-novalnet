...
class NovalnetHelper {
	...
	public static function showPaymentDescription($payment_name) {
		...
		if (($payment_name == 'novalnet_cc') && (MODULE_PAYMENT_NOVALNET_CC_AUTHENTICATE == 'zero_amount') || (MODULE_PAYMENT_NOVALNET_DIRECT_DEBIT_ACH_AUTHENTICATE == 'true') && ($payment_name == 'novalnet_direct_debit_ach')) {
	...
	public static function getTransactionData($receivedData = '') {
		...
		$data['transaction'] = [
			...
			'system_name'      => 'Gambio',
			'system_version'   => $gx_version . '-NN12.2.0',
	...
	*/
	...
	public static function includeAdminJS($payment_code, $authorize = false) {
		if (isset($_GET['module']) && $_GET['module'] == $payment_code && isset($_GET['action']) && $_GET['action'] == 'edit') {
			echo '<script src="' . DIR_WS_CATALOG . 'ext/novalnet/js/novalnet.min.js" type="text/javascript"  integrity="sha384-N+PgkQChyIKyAHpnkAP8N9pNMLqiNdCYOM7yVXIU16ZFUzR1OKVozFXfkKBQlXvo"></script>';
			if ($authorize) {
