...
class NovalnetHelper {
    ...
    public static function merchantValidate($payment_name) {
        ...
        } else if (self::novalnetStringCheck(MODULE_PAYMENT_INSTALLED, $payment_name)  && $payment_name == 'novalnet_invoice' && $invoice_due_date != '' && !is_numeric(trim($invoice_due_date))) {
			return array(MODULE_PAYMENT_DUE_DATE_INVAILD,true,'invoice_due_date');
		} else  if ($payment_name == 'novalnet_sepa' && $sepa_due_date != '' && (!is_numeric(trim($sepa_due_date)) || $sepa_due_date < 2 || $sepa_due_date > 14)) {
		...
		defined('MODULE_PAYMENT_'.strtoupper($payment_name).'_GUARANTEE') == 'True' && (constant('MODULE_PAYMENT_'.strtoupper($payment_name).'_GUARANTEE_MINIMUM_ORDER_AMOUNT') != '' )) {
    ...
    public static function getCommonParms($datas) {
        ...
        $urlparam              = array_merge(array(
            ...
            'system_name' 			=> 'Gambio',
            'system_version' 		=> $gx_version . '-NN(11.1.6)',
