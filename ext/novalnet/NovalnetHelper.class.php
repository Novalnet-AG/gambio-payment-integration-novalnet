...
class NovalnetHelper {
	...
	public static function showPaymentDescription($payment_name) {
		...
		if (($payment_name == 'novalnet_cc') && (defined('MODULE_PAYMENT_NOVALNET_CC_AUTHENTICATE') && MODULE_PAYMENT_NOVALNET_CC_AUTHENTICATE == 'zero_amount')) {
	...
	public static function showSepaDescription($payment_name){
		...
		if (($payment_name == 'novalnet_sepa') && (defined('MODULE_PAYMENT_NOVALNET_SEPA_AUTHENTICATE') && MODULE_PAYMENT_NOVALNET_SEPA_AUTHENTICATE == 'zero_amount')) {
	...
	public static function getTransactionData($receivedData = '') {
		...
		$data['transaction'] = [
			'amount'           => (!empty($receivedData) ? $receivedData['transaction']['amount'] : self::getOrderAmount($order->info['total'])),
			'currency'         => $order->info['currency'],
			'test_mode'        => constant('MODULE_PAYMENT_'. strtoupper($order->info['payment_class']) . '_TEST_MODE') == 'true' ? 1 : 0,
			'system_name'      => 'Gambio',
			'system_version'   => $gx_version . '-NN(12.0.1)',
	...
	public static function getCardDetails($order_no) {
		...
		if (empty($_SESSION['novalnet_cc_token'])) {
			...
			$data['transaction']['enforce_3d'] = ((defined('MODULE_PAYMENT_NOVALNET_CC_3D_SECURE_FORCE') && MODULE_PAYMENT_NOVALNET_CC_3D_SECURE_FORCE == 'true') ? '1' : '0');
	...
	public static function getToeknizationDetails($payment_name, &$transaction_data) {
		if (defined('MODULE_PAYMENT_'.strtoupper($payment_name).'_TOKENIZATION') && constant('MODULE_PAYMENT_'.strtoupper($payment_name).'_TOKENIZATION')=='true') {
	...
	*/
	...
	public static function dateOfBirth($dob) {
		...
		if (time() < strtotime('+18 years', strtotime($dob))) {
			$error_message = defined('MODULE_PAYMENT_GUARANTEE_PAYMENT_AGE_ERROR_MESSAGE') ? MODULE_PAYMENT_GUARANTEE_PAYMENT_AGE_ERROR_MESSAGE : '';
			return $error_message;
		}
	}
	...
	public static function getGuaranteeField($name, $customer_details) {
		$placeholder = defined('MODULE_PAYMENT_NOVALNET_GUARANTEE_DOB_FORMAT') ? MODULE_PAYMENT_NOVALNET_GUARANTEE_DOB_FORMAT : '';
