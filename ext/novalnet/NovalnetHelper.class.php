...
class NovalnetHelper {
	...
	public static function validateMerchantConfiguration() {
		...
		if (isset($_GET['module']) && strpos(MODULE_PAYMENT_INSTALLED, $_GET['module']) && $_GET['module'] == 'novalnet_config' && (!isset($_GET['action']) || $_GET['action'] != 'edit')) {
			if ($error_display) {
				echo self::displayErrorMessage(defined('MODULE_PAYMENT_NOVALNET_CONFIG_TEXT_TITLE') ? MODULE_PAYMENT_NOVALNET_CONFIG_TEXT_TITLE : '');
			}
		}
	}
	...
	public static function showPaymentDescription($payment_name) {
		...
		$payment_description = '<link rel="stylesheet" type="text/css" href="ext/novalnet/css/novalnet.min.css"><div class="novalnet-info-box">';
		...
		if ($payment_name=='novalnet_cc' && defined('MODULE_PAYMENT_NOVALNET_CC_AUTHENTICATE')){
		...
		if ($payment_name == 'novalnet_direct_debit_ach' && defined('MODULE_PAYMENT_NOVALNET_DIRECT_DEBIT_ACH_AUTHENTICATE')){
			if(MODULE_PAYMENT_NOVALNET_DIRECT_DEBIT_ACH_AUTHENTICATE == 'true'){
				$payment_description .= defined('MODULE_PAYMENT_NOVALNET_ZEROAMOUNT_BOOKING_TEXT') ? MODULE_PAYMENT_NOVALNET_ZEROAMOUNT_BOOKING_TEXT : '';
			}
	...
	public static function getTransactionData($receivedData = '') {
		...
		$amount = (!empty($_SESSION['novalnet']['payment_amount']) && empty($_SESSION['novalnet']['wallet_amount']) ? (self::getOrderAmount($_SESSION['novalnet']['payment_amount'])) : (!empty($_SESSION['novalnet']['wallet_amount']) ? sprintf('%0.2f', $_SESSION['novalnet']['wallet_amount']) * 100: 0));
		$data['transaction'] = [
			...
			'system_name'      => 'Gambio',
			'system_version'   => $gx_version . '-NN12.3.1',
	...
	public static function updateTransactionDetails($payment_method, $response, $status = true) {
		if ($status && isset($response['result']['status']) && $response['result']['status'] == 'SUCCESS') {
	...
	public static function getTransactionDetails($response) {
		...
		if ($response['transaction']['payment_type'] == 'MULTIBANCO') {
			...
			$txn_details .= PHP_EOL . sprintf(MODULE_PAYMENT_NOVALNET_PARTNER_SUPPLIER_ID, $response['transaction']['service_supplier_id']) . PHP_EOL;
		}
		return $txn_details;
	}
	...
	public static function getBankDetails($response, $order_no = '') {
		...
		if(!empty($order_no) && $response['transaction']['payment_type'] != 'INSTALMENT_INVOICE') {
			...
			$note .= sprintf(MODULE_PAYMENT_NOVALNET_PAYMENT_REFERENCE, 1, ( $response['transaction']['tid'])) . PHP_EOL;
	...
	public static function storeInstalmentdetails($response, $total_amount)
    {
        ...
        $cycles = isset($instalment['cycle_dates']) ? $instalment['cycle_dates'] : "";
        $cycle_details = array();
		if (!empty($cycles)) {
	...
	public static function formInstalmentPaymentReference($response) {
		$txn_details = '';
		$order_total = isset($_SESSION['novalnet']['payment_amount']) ? self::getOrderAmount($_SESSION['novalnet']['payment_amount']) : 0;
        $total_amount = ($response['instalment']['total_amount'] < $order_total) ? $order_total : $response['instalment']['total_amount'];
		...
		if ($response['result']['status'] == 'SUCCESS') {
			if ($response['transaction']['payment_type'] == 'INSTALMENT_INVOICE' && (in_array($response['transaction']['status'], array('CONFIRMED', 'ON_HOLD')))) {
				...
				$txn_details .= sprintf(MODULE_PAYMENT_NOVALNET_PAYMENT_REFERENCE, 1, ($response['transaction']['tid'])) . PHP_EOL;
	...
	public static function updateOrderStatus($order_id, $txn_details, $response, $payment_method) {
		...
		$novalnet_transaction_details = array(
			...
			'amount'        => isset($response['instalment']['total_amount']) ? $response['instalment']['total_amount'] : $response['transaction']['amount'],
	...
	public static function processTempOrderFail($payment_method, $order_id, $response, $error_text = '') {
		if (!empty($order_id)) {
			...
			$response_tid = isset($response['tid']) ? $response['tid'] : '';
			self::updateTempOrderFail($order_id, $response_tid, $status_text);
			xtc_redirect(xtc_href_link(FILENAME_CHECKOUT_PAYMENT, 'payment_error=' . $payment_method . '&error=' . $status_text, 'SSL', true, false));
		}
	}
	...
	*/
	...
	public static function includeAdminJS($payment_code, $authorize = false) {
		if (isset($_GET['module']) && $_GET['module'] == $payment_code && isset($_GET['action']) && $_GET['action'] == 'edit') {
			echo '<script src="' . DIR_WS_CATALOG . 'ext/novalnet/js/novalnet.min.js" type="text/javascript" integrity="sha384-XpTqEgxNginGQEEsAKXY/Gu4SEBFJgtt+NWp5bhIGrQUVmIQ686g8rzazycrtTiK"></script>';
			if ($authorize) {
	...
	public static function getCustomerWalletData($receivedData = '') {
		...
		$data['customer'] = [
			...
			'first_name'  => (!empty($receivedData) ? $receivedData['order']['billing']['contact']['firstName'] : $order->billing['firstname']),
			'last_name'   => (!empty($receivedData) ? $receivedData['order']['billing']['contact']['lastName'] : $order->billing['lastname']),
			'email'       => (!empty($receivedData['order']['billing']['contact']['email']) ? $receivedData['order']['billing']['contact']['email'] : (!empty($receivedData['order']['shipping']['contact']['email']) ? $receivedData['order']['shipping']['contact']['email'] : $order->customer['email_address'])),
			'customer_ip' => self::getIPAddress(),
			'customer_no' => $_SESSION['customer_id'],
			'billing'     => [
				'street'            => (!empty($receivedData) ? $receivedData['order']['billing']['contact']['addressLines'] : $order->billing['street_address']),
