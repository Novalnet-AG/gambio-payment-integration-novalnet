...
class NovalnetHelper {
	...
	public static function showSepaDescription($payment_name){
		...
		if($_SESSION['language_code'] == 'en'){
			return sprintf(
							'<div class ="novalnet-info-box">' . $test_mode . '<span style="list-style-type:disc">'.MODULE_PAYMENT_NOVALNET_SEPA_DESC.'</span><a id="%s_mandate" style="cursor:pointer;" 
							...
							'<div><span style="color:#00A9E0; list-style-type:disc">I hereby grant the mandate for the SEPA direct debit (electronic transmission) and confirm that the given bank details are correct!</span>',
							$payment_name,
			...
			return sprintf(
							'<div class ="novalnet-info-box"><span style="list-style-type:disc">'.MODULE_PAYMENT_NOVALNET_SEPA_DESC.'</span><a id="%s_mandate" style="cursor:pointer;" 
							onclick="%s">%s</a><div class="info novalnet-display-none" 
							id="%s_about_mandate" 
							style="display:none;"><p>%s</p><p><strong>%s</strong></p><p><strong>%s</strong>%s</p></div>',
							$payment_name,
							"jQuery('#" . $payment_name . "_about_mandate').toggle('slow')",						    
							'<div><span style="color:#00A9E0; list-style-type:disc"> Ich erteile hiermit das SEPA-Lastschriftmandat (elektronische Übermittlung) und bestätige, dass die Bankverbindung korrekt ist!</span>',
							$payment_name,
	...
	/**
	 * Test mode notification for the sepa payments
	 *
	 * @param $payment_name
	 *
	 * @return $payment_description
	 */
	public static function test_mode_notification($payment_name){		
		if (constant('MODULE_PAYMENT_'. strtoupper($payment_name) . '_TEST_MODE') == 'true'){
			return '<div class="novalnet-test-mode">testmode</div>';
		}
	}
	...
	public static function getTransactionData() {
		...
		$data['transaction'] = [
			...
			'system_name'      => 'Gambio',
			'system_version'   => $gx_version . '-NN(12.0.0)',
	...
	public static function updateOrderStatus($order_id, $txn_details, $response, $payment_method) {
		...
		&& (constant('MODULE_PAYMENT_'. strtoupper($payment_method) . '_TOKENIZATION') == 'true')) {
			$novalnet_transaction_details['payment_details'] = json_encode($response['transaction']['payment_data']);
			// Delete duplicate tokens in novalnet table
			if(in_array($response['transaction']['payment_type'], array('DIRECT_DEBIT_SEPA', 'GUARANTEED_DIRECT_DEBIT_SEPA', 'INSTALMENT_DIRECT_DEBIT_SEPA', 'CREDITCARD'))){
	...
	public static function checkDuplicateToken($customerId, $response) {
		...
		if (in_array($response['transaction']['payment_type'], array('DIRECT_DEBIT_SEPA', 'GUARANTEED_DIRECT_DEBIT_SEPA', 'INSTALMENT_DIRECT_DEBIT_SEPA'))) {
			$payment_details = xtc_db_query("SELECT payment_details, id, order_no FROM novalnet_transaction_detail WHERE customer_id='" . xtc_db_input($customerId) . "' AND payment_details !='' AND status = 'CONFIRMED' AND payment_type in ('DIRECT_DEBIT_SEPA', 'GUARANTEED_DIRECT_DEBIT_SEPA', 'INSTALMENT_DIRECT_DEBIT_SEPA')");
			...
			$payment_details = xtc_db_query("SELECT payment_details, id, order_no FROM novalnet_transaction_detail WHERE customer_id='" . xtc_db_input($customerId) . "' AND payment_details !='' AND status = 'CONFIRMED' AND payment_type = '{$response['transaction']['payment_type']}'");
		...
		while ($payment_detail = xtc_db_fetch_array($payment_details)) {
			...
			if(($check_string == $check_data) && !empty($payment_detail['id'])) {
				$payment_details = xtc_db_query("UPDATE novalnet_transaction_detail SET payment_details = '' WHERE id = {$payment_detail['id']} AND order_no = {$payment_detail['order_no']} AND customer_id = {$customerId}");
			}
		}
	}
	...
	public static function updateTempOrderFail($order_id, $tid, $status_text) {
		...
		$note .= $status_text;
		xtc_db_query("UPDATE " . TABLE_ORDERS . " SET orders_status = " . MODULE_PAYMENT_NOVALNET_ONHOLD_ORDER_CANCELLED .", comments = '$note' WHERE orders_id='$order_id'");
