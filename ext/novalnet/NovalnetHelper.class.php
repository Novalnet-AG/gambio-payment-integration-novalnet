...
class NovalnetHelper {
    ...
    public static function merchantValidate($payment_name) {
        ...
        $value   = MODULE_PAYMENT_NOVALNET_VENDOR_ID . '|' . MODULE_PAYMENT_NOVALNET_PRODUCT_ID . '|' . MODULE_PAYMENT_NOVALNET_TARIFF_ID . '|' . MODULE_PAYMENT_NOVALNET_AUTHCODE . '|' . MODULE_PAYMENT_NOVALNET_PAYMENT_ACCESS_KEY . '|' . MODULE_PAYMENT_NOVALNET_REFERRER_ID . '|' . MODULE_PAYMENT_NOVALNET_GATEWAY_TIMEOUT . '|' . MODULE_PAYMENT_NOVALNET_TARIFF_PERIOD2_AMOUNT . '|' . MODULE_PAYMENT_NOVALNET_TARIFF_PERIOD2 . '|' . MODULE_PAYMENT_NOVALNET_TARIFF_PERIOD;
        ...
        } else if ( self::novalnetStringCheck(MODULE_PAYMENT_INSTALLED, $payment_name)  && in_array($payment_name, array( 'novalnet_invoice', 'novalnet_cc','novalnet_paypal','novalnet_sepa' )) && defined('MODULE_PAYMENT_'.strtoupper($payment_name).'_MANUAL_CHECK_LIMIT') && constant('MODULE_PAYMENT_'.strtoupper($payment_name).'_MANUAL_CHECK_LIMIT') != '' && !is_numeric(constant('MODULE_PAYMENT_'.strtoupper($payment_name).'_MANUAL_CHECK_LIMIT'))) {
		...
		} else  if ($payment_name == 'novalnet_cashpayment' && MODULE_PAYMENT_NOVALNET_CASHPAYMENT_DUE_DATE != '' && !is_numeric(trim(MODULE_PAYMENT_NOVALNET_CASHPAYMENT_DUE_DATE))) {
		...
		constant('MODULE_PAYMENT_'.strtoupper($payment_name).'_GUARANTEE') == 'True' && (constant('MODULE_PAYMENT_'.strtoupper($payment_name).'_GUARANTEE_MINIMUM_ORDER_AMOUNT') != '' )) {
			if(trim(constant('MODULE_PAYMENT_'.strtoupper($payment_name).'_GUARANTEE_MINIMUM_ORDER_AMOUNT')) != '') {
				if (!preg_match('/^\d+$/',constant('MODULE_PAYMENT_'.strtoupper($payment_name).'_GUARANTEE_MINIMUM_ORDER_AMOUNT'))) {
					return array(MODULE_PAYMENT_NOVALNET_AMOUNT_ERROR_MESSAGE,true,'amount_invaild');
				} else if(constant('MODULE_PAYMENT_'.strtoupper($payment_name).'_GUARANTEE_MINIMUM_ORDER_AMOUNT') < 2000) {
					return array(constant('MODULE_PAYMENT_'.strtoupper($payment_name).'_GUARANTEE_MINIMUM_AMOUNT_ERROR'),true,'guarantee_minimum');
				}
			}
    ...
    public static function getCommonParms($datas) {
        ...
        $urlparam              = array_merge(array(
            ...
            'system_name' 			=> 'Gambio',
            'system_version' 		=> $gx_version . '-NN(11.1.4)',
    ...
    public static function checkPaymentStatus($payment_name, $response,$payment_param = '') {
        ...
        if ($response['status'] == 100 || ($payment_name == 'novalnet_paypal' && $response['status'] == 90)) { // Payment Success
            ...
            if (in_array($payment_name, array( 'novalnet_invoice', 'novalnet_prepayment' ))) {
				...
				$cashpayment_slip_details = array_merge($nearest_store,array('due_date'       => $response['cashpayment_due_date'],'cp_checkout_token'=> $response['cp_checkout_token']));                
    ...
    public static function updateOrderStatus($order_id, $payment_name) {
        ...
        if (in_array($payment_name, array( 'novalnet_invoice','novalnet_prepayment' ))) {
            ...
            if(!in_array($_SESSION['novalnet'][$payment_name]['gateway_response']['tid_status'], array(75,91))){
        ...
        $payment_order_status = in_array($_SESSION['novalnet'][$payment_name]['gateway_response']['tid_status'], array(75,85,86,90)) ? constant('MODULE_PAYMENT_'.strtoupper($payment_name).'_PENDING_ORDER_STATUS') : (($_SESSION['novalnet'][$payment_name]['gateway_response']['payment_id'] == 41) ? constant('MODULE_PAYMENT_NOVALNET_INVOICE_CALLBACK_ORDER_STATUS') : constant('MODULE_PAYMENT_' . strtoupper($payment_name) . '_ORDER_STATUS'));
	...
	public static function transactionCommentsForm ($response,$paymentname) {
		...
		if(in_array($response['payment_id'],array('41')) && in_array($response['tid_status'], array('75'))){
			$trans_comments .=  MODULE_PAYMENT_NOVALNET_MENTION_GUARANTEE_PAYMENT_PENDING_TEXT.PHP_EOL;
		}
		if(!empty($response['subs_id'])){
			$charge_date = ($response['next_subs_cycle']) ? $response['next_subs_cycle'] : $response['paid_until'];
			$createDate = new DateTime($charge_date);
			$charge_date = $createDate->format('d.m.Y');
			$trans_comments .= sprintf(MODULE_PAYMENT_NOVALNET_CALLBACK_CHARGING_DATE_COMMENTS, $charge_date).PHP_EOL; 
		} 
