...
class NovalnetValidator
{
    ...
    public function check_novalnet_payment(string $payment)
    {
        ...
        if ($this->helper->checkMerchantCredentials() || strpos(MODULE_PAYMENT_INSTALLED, 'novalnet_config') === false
            || (!in_array($payment, ['novalnet_instalment_invoice', 'novalnet_instalment_sepa', 'novalnet_guarantee_invoice', 'novalnet_guarantee_sepa']) && !$this->helper->hidePaymentVisibility($payment)) || !$this->checkGuarantee($payment)
        ) {
    ...
    public function checkGuarantee(string $code)
    {
        ...
        $minimumAmount = (defined("MODULE_PAYMENT_{$payment_last_name}_MINIMUM_ORDER_AMOUNT") && constant("MODULE_PAYMENT_{$payment_last_name}_MINIMUM_ORDER_AMOUNT") > '0') ? constant("MODULE_PAYMENT_NOVALNET_GUARANTEE_{$payment_last_name}_MINIMUM_ORDER_AMOUNT") : (in_array($code, ['novalnet_instalment_sepa', 'novalnet_instalment_invoice']) ? '1998' : '999');
