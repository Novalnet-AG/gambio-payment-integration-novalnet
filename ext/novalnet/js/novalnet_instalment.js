...
$("#novalnet_global_recurring_period_cycles_sepa option:selected").change(function(){
	instalment_table();
});
$("#novalnet_global_recurring_period_cycles_invoice option:selected").change(function(){
	instalment_table();
});
...
function novalnet_instalment_sepa_load() {
	...
	if (jQuery("input[name='payment']").val() == 'novalnet_instalment_sepa'){
		$(".about_mandate").mouseover(function() {
			//$(".mandate_confirm").next().toggle('slow');
