 ...
 */
 function doCancelSubscription($subscription_params,$order_id) {
        ...
        if ($data['status'] == 100) { // Success of subscription cancellation
            ...
            $subscription_orders = xtc_db_fetch_array($subscription_orders);
            // Update the order status in shop
				...
				), 'update', 'orders_id="' . $subscription_orders['order_no'] . '"');

                xtc_db_query("INSERT INTO " . TABLE_ORDERS_STATUS_HISTORY . " SET orders_id =".(int)$subscription_orders['order_no'].",orders_status_id = '".MODULE_PAYMENT_NOVALNET_SUBSCRIPTION_CANCEL."', date_added = NOW(), customer_notified = '1', comments = '".$message."'");
