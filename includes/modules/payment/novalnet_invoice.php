...
class novalnet_invoice {
				...
				$selection['fields'][] = array(
							...
							<script type="text/javascript">
								...
								jQuery(document).ready(function() {
									j("#novalnet_invoice_dob").datepicker({dateFormat: "yy-mm-dd", changeMonth: true, changeYear: true, yearRange: "-100:+0",},j.datepicker.regional[ jQuery("#nn_shop_lang").val()]);
									j("#novalnet_invoice_dob").click(function(){
    ...
    function before_process() {
        ...
        if (!empty($_SESSION['novalnet'][$this->code]['tid']) && $_SESSION['novalnet'][$this->code . '_newpin'] != 1 && $this->fraud_module != 'False' && empty($_SESSION['novalnet'][$this->code]['guarantee'])) {
            ...
            if ((trim(MODULE_PAYMENT_NOVALNET_INVOICE_MANUAL_CHECK_LIMIT) > 0 && $order_amount >= trim(MODULE_PAYMENT_NOVALNET_INVOICE_MANUAL_CHECK_LIMIT) && MODULE_PAYMENT_NOVALNET_INVOICE_AUTHENTICATE === 'authorize' ) || (empty (MODULE_PAYMENT_NOVALNET_INVOICE_MANUAL_CHECK_LIMIT) && MODULE_PAYMENT_NOVALNET_INVOICE_AUTHENTICATE == 'authorize' )){ // If condition true then order proceed on-hold transaction
                $urlparam['on_hold'] = 1;
            }
			...
			if ((!empty($_SESSION['novalnet'][$this->code]['guarantee']) && !empty($_SESSION['novalnet'][$this->code]['novalnet_invoice_dob']) && (time() > strtotime('+18 years', strtotime(date('Y-m-d',strtotime($_SESSION['novalnet'][$this->code]['novalnet_invoice_dob'])))) && NovalnetHelper::validateDuedate(trim($_SESSION['novalnet'][$this->code]['novalnet_invoice_dob'])))) || (!empty($_SESSION['novalnet'][$this->code]['guarantee']) && $order->billing['company'] != '')) { // To process guarantee payments 
				$urlparam['key']          = 41;
