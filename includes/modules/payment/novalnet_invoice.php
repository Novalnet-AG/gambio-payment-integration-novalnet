...
class novalnet_invoice {
    ...
    public function __construct() {
        ...
        if (strpos(MODULE_PAYMENT_INSTALLED, $this->code) !== false) {
            ...
            $this->enabled      = ((defined('MODULE_PAYMENT_NOVALNET_INVOICE_ENABLE_MODULE') && MODULE_PAYMENT_NOVALNET_INVOICE_ENABLE_MODULE == 'True') ? true : false);
            $this->fraud_module = ((defined('MODULE_PAYMENT_NOVALNET_INVOICE_ENABLE_FRAUDMODULE') && MODULE_PAYMENT_NOVALNET_INVOICE_ENABLE_FRAUDMODULE == 'False') ? false : true);
    ...
    function selection() {
			...
			$selection  = array(
					'id' => $this->code,
					'module' => $title,
					'description' => MODULE_PAYMENT_NOVALNET_INV_PRE_DESC . '<br>' . $test_mode .'<br>'. $endcustomerinfo.'<br><span style="color:red">'.$gurantee_error.'</span><br><input type="hidden" id="nn_root_invoice_catalog" value="' . DIR_WS_CATALOG . '"/><script type= text/javascript src="' . DIR_WS_CATALOG . 'ext/novalnet/js/novalnet_invoice.js"></script><noscript><input type="hidden" name="js_enabled" value=1 ><br /></noscript>');
			if($shop_version > '3810') {
    ...
    function before_process() {
        ...
        if (!empty($_SESSION['novalnet'][$this->code]['tid']) && $_SESSION['novalnet'][$this->code . '_newpin'] != 1) {
            ...
            if (trim(MODULE_PAYMENT_NOVALNET_INVOICE_MANUAL_CHECK_LIMIT) > 0 && $order_amount >= trim(MODULE_PAYMENT_NOVALNET_INVOICE_MANUAL_CHECK_LIMIT)) { // If condition true then order proceed on-hold transaction
                $urlparam['on_hold'] = 1;
            }
