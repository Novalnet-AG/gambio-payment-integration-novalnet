...
class novalnet_instantbank {
    ...
    public function __construct() {
        ...
        $this->title        = defined('MODULE_PAYMENT_NOVALNET_INSTANTBANK_TEXT_TITLE') ? 'Novalnet '. MODULE_PAYMENT_NOVALNET_INSTANTBANK_TEXT_TITLE : '';
        $this->public_title = defined('MODULE_PAYMENT_NOVALNET_INSTANTBANK_PUBLIC_TITLE') ? MODULE_PAYMENT_NOVALNET_INSTANTBANK_PUBLIC_TITLE : '';
        $this->sort_order   = 0;
        if (strpos(MODULE_PAYMENT_INSTALLED, $this->code) !== false) {
            ...
            $this->test_mode       = ((defined('MODULE_PAYMENT_NOVALNET_INSTANTBANK_TEST_MODE') && MODULE_PAYMENT_NOVALNET_INSTANTBANK_TEST_MODE == 'true') ? true : false); // To check the test mode
            $this->tmpOrders       = true;
    ...
    function selection() {
        ...
        $selection  = array(
                'id'		  => $this->code,
                'module' 	  => $title,
                'description' => MODULE_PAYMENT_NOVALNET_INSTANTBANK_REDIRECT_DESC . '<br>' . $test_mode . '<br><span id ="notification_buyer_wrap">' . $endcustomerinfo.'</span><br>');
		if($shop_version > '3810') {
			if((!defined('MODULE_PAYMENT_NOVALNET_PAYMENT_LOGO_DISPLAY')) || MODULE_PAYMENT_NOVALNET_PAYMENT_LOGO_DISPLAY == 'true'){
				$selection['logo_url'] = xtc_href_link('images/icons/payment/novalnet_instantbank.png', '', 'SSL', false, false, false, true, true);
			}else {
				$selection['logo_url'] = '';
				$selection['logo_alt'] = '';
			}
    ...
    function payment_action() {
        ...
        if (isset($_SESSION['novalnet'][$this->code]['payment_amount'])) { // Generate the redirect params
            ...
            $urlparam['payment_type']     = 'ONLINE_TRANSFER';
    ...
    function check() {
        ...
        if($_SESSION['GX'] == '1'){
			  $check_query  = xtc_db_query("select `value` from `gx_configurations` where `key` = 'configuration/MODULE_PAYMENT_NOVALNET_INSTANTBANK_ALLOWED'");
    ...
    function install() {
		if($_SESSION['GX'] == '1'){
			...
			xtc_db_query("insert into `gx_configurations` ( `key`, `value`,   `sort_order`,  `last_modified`) values ('configuration/MODULE_PAYMENT_NOVALNET_INSTANTBANK_ALLOWED', '', '1',  now())");
			xtc_db_query("insert into `gx_configurations` ( `key`, `value`,   `sort_order`, `last_modified`) values ('configuration/MODULE_PAYMENT_NOVALNET_INSTANTBANK_ALIAS', 'NN_IB', '0', now())");
			xtc_db_query("insert into `gx_configurations` ( `key`, `value`,   `sort_order`, `type`, `last_modified`) values ('configuration/MODULE_PAYMENT_NOVALNET_INSTANTBANK_STATUS','false', '2', 'switcher',now())");
			xtc_db_query("insert into `gx_configurations` ( `key`, `value`,   `sort_order`, `type`, `last_modified`) values ('configuration/MODULE_PAYMENT_NOVALNET_INSTANTBANK_TEST_MODE','false', '3','switcher', now())");
			xtc_db_query("insert into `gx_configurations` ( `key`, `value`,   `sort_order`,  `last_modified`) values ('configuration/MODULE_PAYMENT_NOVALNET_INSTANTBANK_VISIBILITY_BY_AMOUNT', '', '4', now())");
			xtc_db_query("insert into `gx_configurations` ( `key`, `value`,   `sort_order`,  `last_modified`) values ('configuration/MODULE_PAYMENT_NOVALNET_INSTANTBANK_ENDCUSTOMER_INFO', '', '5', now())");
			xtc_db_query("insert into `gx_configurations` ( `key`, `value`,   `sort_order`,  `last_modified`) values ('configuration/MODULE_PAYMENT_NOVALNET_INSTANTBANK_SORT_ORDER', '0', '6',  now())");
			xtc_db_query("insert into `gx_configurations` ( `key`, `value`,   `sort_order`, `type`, `last_modified`) values ('configuration/MODULE_PAYMENT_NOVALNET_INSTANTBANK_ORDER_STATUS', '0',  '7', 'order-status',now())");
			xtc_db_query("insert into `gx_configurations` ( `key`, `value`,   `sort_order`, `type`, `last_modified`) values ('configuration/MODULE_PAYMENT_NOVALNET_INSTANTBANK_PAYMENT_ZONE', '0', '8', 'geo-zone',now())");
        ...
        ('MODULE_PAYMENT_NOVALNET_INSTANTBANK_STATUS','false', '6', '2', 'xtc_mod_select_option(array(\'true\' => MODULE_PAYMENT_NOVALNET_TRUE,\'false\' => MODULE_PAYMENT_NOVALNET_FALSE),\'MODULE_PAYMENT_NOVALNET_INSTANTBANK_STATUS\'," .'MODULE_PAYMENT_NOVALNET_INSTANTBANK_STATUS'. ",' , '',now()),
    ...
    function remove() {
		if($_SESSION['GX'] == '1' ){
			xtc_db_query("delete from `gx_configurations` where `key` in ('" . implode("', '", array_merge($this->keys(),array('configuration/MODULE_PAYMENT_NOVALNET_INSTANTBANK_ALIAS'))) . "')");
		}else{
    ...
    function keys() {
		...
		if($_SERVER['REQUEST_URI'] === DIR_WS_CATALOG.'admin/modules.php?set=payment&module=novalnet_instantbank&action=edit'){
		  echo '<script src="' . DIR_WS_CATALOG . 'ext/novalnet/js/novalnet.js" type="text/javascript"></script>';
	    }
	     if($_SESSION['GX'] == '1' ){
			 $alias_menu = array();
		    if($gx_version >= '3.1.1.0' ){
			    $alias_menu = array_merge($alias_menu,array('configuration/MODULE_PAYMENT_NOVALNET_INSTANTBANK_ALIAS'));
		    }
          ...
          $alice_val = 'configuration/MODULE_PAYMENT_NOVALNET_INSTANTBANK_ALIAS';
		}else{
			$alias_menu = array();
		   if($gx_version >= '3.1.1.0' ) {
		...
		$return_key =  array_merge(array(
            ...
            'MODULE_PAYMENT_NOVALNET_INSTANTBANK_STATUS',
          ...
          $alice_val = 'MODULE_PAYMENT_NOVALNET_INSTANTBANK_ALIAS';
		}
