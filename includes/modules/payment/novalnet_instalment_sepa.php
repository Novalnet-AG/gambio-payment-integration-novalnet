...
class novalnet_instalment_sepa
{
	...
	function selection() {
		...
		$selection = [
			...
			'description' => '<link rel="stylesheet" type="text/css" href="ext/novalnet/css/novalnet.css">'.NovalnetHelper::showSepaDescription($this->code) . $this->info
								...
								. '<script type=text/javascript src="' . DIR_WS_CATALOG . 'ext/novalnet/js/novalnet_instalment_sepa.js"></script>'
								. '<input type="hidden" name="nn_instalmentsepa_birthdate_error" id="nn_instalmentsepa_birthdate_error" value="'.MODULE_PAYMENT_NOVALNET_GUARANTEE_DOB_ERROR_MSG.'">',
		];
		...
		if (MODULE_PAYMENT_NOVALNET_INSTALMENT_SEPA_TOKENIZATION == 'true') {
			...
			if (!empty($instasaved_card_details)) {
				...
				foreach ($instasaved_card_details as $key => $value) {
					...
					$oneclick  = (!empty($instasaved_card_details)) ? '<nobr><input name = "novalnet_instalment_sepa_token" type="radio" class ="novalnet_instalment_sepa_saved_acc" value="'.$value['token'].'" onclick/>'.sprintf(('IBAN %s'),$value['iban']) . '&nbsp;&nbsp;<a href="javascript:void(0);" id="'.$value['id'].'" class="instalment_token" >Delete</a></nobr>' : '';
					if(!empty($value['iban'])){
				...
				$selection['fields'][] = ['title' => '<span id="instalment_iban" style="display:none;">'.MODULE_PAYMENT_NOVALNET_SEPA_IBAN.'</span>'. xtc_draw_input_field('novalnet_instalment_sepa_iban', '', 'id="novalnet_instalment_sepa_iban_field" autocomplete="off" placeholder ="DE00 0000 0000 0000 0000 00" style="display:none;" onkeypress="return NovalnetUtility.formatIban(event,\'novalnet_instalment_sepa_bic\');" onkeyup="return NovalnetUtility.formatIban(event,\'novalnet_instalment_sepa_bic\');" onchange="return NovalnetUtility.formatIban(event,\'novalnet_instalment_sepa_bic\');"'),
				];
	...
	function pre_confirmation_check() {
		...
		if (!empty($_REQUEST['novalnet_instalment_sepa_iban'])) {
			...
			if (MODULE_PAYMENT_NOVALNET_INSTALMENT_SEPA_TOKENIZATION == 'true' && !empty($_REQUEST['nn_instalment_sepa_checkbox']))
			{
				$_SESSION['novalnet_instalment_sepa_create_token'] = $_REQUEST['nn_instalment_sepa_checkbox'];
			}
		} elseif (!empty($_REQUEST['novalnet_instalment_sepa_token']) && ($_REQUEST['novalnet_instalment_sepa_token'] != 'new') &&MODULE_PAYMENT_NOVALNET_INSTALMENT_SEPA_TOKENIZATION == 'true') {
		...
		&& MODULE_PAYMENT_NOVALNET_INSTALMENT_SEPA_ALLOW_B2B == 'false')) {
	...
	function before_process() {
		...
		if (MODULE_PAYMENT_NOVALNET_INSTALMENT_SEPA_AUTHENTICATE == 'true'
		&& (NovalnetHelper::getOrderAmount($order->info['total']) >= MODULE_PAYMENT_NOVALNET_INSTALMENT_SEPA_MANUAL_CHECK_LIMIT)) {
