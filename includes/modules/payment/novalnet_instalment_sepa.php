...
class novalnet_instalment_sepa
{
	...
	function selection() {
		...
		if (defined('MODULE_PAYMENT_NOVALNET_INSTALMENT_SEPA_TOKENIZATION') && MODULE_PAYMENT_NOVALNET_INSTALMENT_SEPA_TOKENIZATION == 'true') {
			...
			if (!empty($instasaved_card_details)) {
				...
				foreach ($instasaved_card_details as $key => $value) {
					...
					$oneclick  = (!empty($instasaved_card_details)) ? '<nobr class="sepa_saved_card_info"><input name = "novalnet_instalment_sepa_token" type="radio" class ="novalnet_instalment_sepa_saved_acc" value="'.$value['token'].'" '.$checked.' onclick/>'.sprintf(('IBAN %s'),$value['iban']) . '&nbsp;&nbsp;<a href="javascript:void(0);" id="'.$value['id'].'" class="instalment_token" >'.(defined('MODULE_PAYMENT_NOVALNET_REMOVE_DUPLICATE_TOKEN') ? MODULE_PAYMENT_NOVALNET_REMOVE_DUPLICATE_TOKEN : '').'</a></nobr>' : '';
					if(!empty($value['iban'])){
		...
		$selection['fields'][] = ['title' => (sprintf(( '<nobr class="novalnet_instalment_text">' . MODULE_PAYMENT_NOVALNET_INSTALLMENT_TEXT . '</nobr>'), xtc_format_price_order($order_amount/100, 1, $order->info['currency'])))];
	...
	function pre_confirmation_check() {
		...
		$_SESSION['novalnet_instalment_sepa_birthdate'] = isset($_REQUEST['novalnet_instalment_sepabirthdate']) ? date('Y-m-d',strtotime($_REQUEST['novalnet_instalment_sepabirthdate'])) : "";
		...
		if (isset($error_message) && $error_message != '') {
			xtc_redirect(xtc_href_link(FILENAME_CHECKOUT_PAYMENT, 'payment_error=' . $this->code . '&error=' . $error_message, 'SSL', true, false));
		}
	}
