...
class novalnet_cc {
    ...
    function selection()     {
        ...
        $test_mode      .=  MODULE_PAYMENT_NOVALNET_CC_SHOP_TYPE == 'ZEROAMOUNT' ? '<br>'.MODULE_PAYMENT_NOVALNET_ZERO_AMOUNT_TEXT : '';
    ...
    function before_process() {
        ...
        if (isset($post['tid'])) {
            ...
            // To process on hold product
             if ((trim(MODULE_PAYMENT_NOVALNET_CC_MANUAL_CHECK_LIMIT) > 0 && $urlparam['amount'] >= trim(MODULE_PAYMENT_NOVALNET_CC_MANUAL_CHECK_LIMIT) && MODULE_PAYMENT_NOVALNET_CC_AUTHENTICATE === 'authorize' ) || (empty (MODULE_PAYMENT_NOVALNET_CC_MANUAL_CHECK_LIMIT) && MODULE_PAYMENT_NOVALNET_CC_AUTHENTICATE == 'authorize' )){ // If condition true then order proceed on-hold transaction
                $urlparam['on_hold'] = 1;
            }
