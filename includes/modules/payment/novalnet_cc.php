...
class novalnet_cc {
	var $code, $title, $enabled, $sort_order, $test_mode, $inline_form;
	...
	function selection() {
		...
		$selection['fields'][] = [
			'title' => '<script src="https://cdn.novalnet.de/js/v2/NovalnetUtility.js"></script>
						<script src="' . DIR_WS_CATALOG . 'ext/novalnet/js/novalnet_cc.js' . '" type="text/javascript"></script>'
		];
		if (MODULE_PAYMENT_NOVALNET_CC_TOKENIZATION == 'true') {
			if (!empty($saved_card_details)) {
				...
				foreach ($saved_card_details as $key => $value) {
					...
					if (in_array($check_string, $payment_data)) {
						continue;
					}
					if (empty($value)) {
						continue;
					...
					$oneclick  = (!empty($saved_card_details)) ? '<nobr><input name = "novalnet_cc_token" type="radio" class ="novalnet_cc_saved_acc" value="'.$value['token'].'" onclick/>'.sprintf(MODULE_PAYMENT_NOVALNET_CC_TOKEN_TEXT,$brand, $last4_digit, $value['card_expiry_month'], $expiry_month) . '&nbsp;&nbsp;<a href="javascript:void(0);" id="'.$value['id'].'" class="cc_token" >Delete</a></nobr>' : '';
		...
		if (MODULE_PAYMENT_NOVALNET_CC_TOKENIZATION == 'true' && (!empty($saved_card_details))) {
		...
		}else if(MODULE_PAYMENT_NOVALNET_CC_TOKENIZATION == 'true' && (empty($saved_card_details))){
	...
	function pre_confirmation_check() {
		...
		if (!empty($_REQUEST['nn_pan_hash']) && !empty($_REQUEST['nn_uniqueid'])) {
			...
			if (MODULE_PAYMENT_NOVALNET_CC_TOKENIZATION == 'true' && !empty($_REQUEST['nn_cc_onclick'])) {
		...
		&& MODULE_PAYMENT_NOVALNET_CC_TOKENIZATION == 'true') {
	...
	function doPaymentCall($order_no = '') {
		...
		if (MODULE_PAYMENT_NOVALNET_CC_AUTHENTICATE == 'true'
		&& NovalnetHelper::getOrderAmount($order->info['total']) >= MODULE_PAYMENT_NOVALNET_CC_MANUAL_CHECK_LIMIT) { // Authorize transaction
	...
	function install() {
		...
		xtc_db_query("insert into `gx_configurations` (`key`, `value`, `sort_order`, `type`, `last_modified`) values ('configuration/MODULE_PAYMENT_NOVALNET_CC_AUTHENTICATE','false', '9' ,'switcher', now())");
	...
	function getNovalnetCCFormDetails() {
		...
		$iframedata = array(
			...
			'enforce_3d'             => (MODULE_PAYMENT_NOVALNET_CC_3D_SECURE_FORCE == 'true') ? '1' : '0',
