...
class novalnet_applepay {
	var $code, $title, $enabled, $sort_order, $test_mode, $description, $info , $tmpOrders;
	...
	function install() {
		...
		xtc_db_query("insert into `gx_configurations` (`key`, `value`, `sort_order`, `last_modified`) values ('configuration/MODULE_PAYMENT_NOVALNET_APPLEPAY_BUTTON_HEIGHT', '40', '11',  now())");
	...
	function applepayCart_page_button() {
		...
		&& (!isset($_SESSION['allow_checkout']) || $_SESSION['allow_checkout'] == 'true') && (strpos(MODULE_PAYMENT_INSTALLED, 'novalnet_config') == true && MODULE_PAYMENT_NOVALNET_SIGNATURE !='' && MODULE_PAYMENT_NOVALNET_PAYMENT_ACCESS_KEY !='' && MODULE_PAYMENT_NOVALNET_TARIFF_ID !=''))
		{
		   ...
		   echo '<script src="' . DIR_WS_CATALOG . 'ext/novalnet/js/novalnet_wallet.min.js" type="text/javascript" integrity="sha384-2bN8cXWW1oVWnhst5+reVjZusq5hesa2kxWr6XYYEyJpXSWLswWgK9qxC6ZuwQzA"></script>'.xtc_draw_hidden_field('nn_applepay_total_label', (defined('MODULE_PAYMENT_NOVALNET_APPLEPAY_TOTAL_LABEL') ? MODULE_PAYMENT_NOVALNET_APPLEPAY_TOTAL_LABEL : ''), 'id="nn_applepay_total_label"'). $this->getParam($order).'<input type="hidden" id = novalnet_applepay_enable value="'.$this->enabled.'"/><script src="https://ajax.googleapis.com/ajax/libs/jquery/3.5.1/jquery.min.js"></script><script src="https://cdn.novalnet.de/js/v3/payment.js"></script></script>';
		}
	}
	...
	function applepayProduct_page_button() {
		...
		&& (strpos(MODULE_PAYMENT_INSTALLED, 'novalnet_config') == true && MODULE_PAYMENT_NOVALNET_SIGNATURE !='' && MODULE_PAYMENT_NOVALNET_PAYMENT_ACCESS_KEY !='' && MODULE_PAYMENT_NOVALNET_TARIFF_ID !='')) {
			...
			echo '<input type="hidden" id="nn_hidden_field" value="'.$products_qty.'"/>';
		}
	}
	...
	function getParam($nn_order = '') {
		...
		$data = array(
			 ...
			 'applepay_button_height'   => defined('MODULE_PAYMENT_NOVALNET_APPLEPAY_BUTTON_HEIGHT') && !empty(MODULE_PAYMENT_NOVALNET_APPLEPAY_BUTTON_HEIGHT) ? MODULE_PAYMENT_NOVALNET_APPLEPAY_BUTTON_HEIGHT : '40',
		...
		if (empty($nn_order) && !empty($order->products)) {
		...
		if(empty($articleDetails) && !empty($GLOBALS['product']->data['products_id'])) {
			...
			$tax_amount = isset($order->info['tax']) ? $order->info['tax'] : $tax;
		...
		if ($current_page[0] == 'checkout_payment' || !empty($nn_order)) {
			...
			{
				$articleDetails[] = array('label'=>  (defined('MODULE_PAYMENT_NOVALNET_APPLEPAY_SHIPPING_LABEL') ? MODULE_PAYMENT_NOVALNET_APPLEPAY_SHIPPING_LABEL : ''), 'amount' => (string)(round(($nn_order->info['shipping_cost'] * $nn_order->info['currency_value']), 2) * 100), 'type' => 'SUBTOTAL');
			}
			...
			if(!empty($nn_order->info['deduction']) || ($_SESSION['customers_status']['customers_status_discount'] != '0')) {		// To add discount
				...
				$articleDetails[] = array('label'=> (defined('MODULE_PAYMENT_NOVALNET_APPLEPAY_DISCOUNT_AND_GIFT_VOUCHER_LABEL') ? MODULE_PAYMENT_NOVALNET_APPLEPAY_DISCOUNT_AND_GIFT_VOUCHER_LABEL : ''), 'amount' => (string) (round($deduction, 2) * 100), 'type' => 'SUBTOTAL');
			}
		...
		if ($current_page[0] == 'product_info') {
			if (!empty($_SESSION['nn_order']) && !empty($_SESSION['nn_order']->info['deduction'])) {
				...
				$articleDetails[] = array('label'=> (defined('MODULE_PAYMENT_NOVALNET_APPLEPAY_DISCOUNT_AND_GIFT_VOUCHER_LABEL') ? MODULE_PAYMENT_NOVALNET_APPLEPAY_DISCOUNT_AND_GIFT_VOUCHER_LABEL : ''), 'amount' => (string) (round($_SESSION['nn_order']->info['deduction'], 2) * 100), 'type' => 'SUBTOTAL');
			} elseif (!empty($_SESSION['gift_vouchers'])) {
				foreach ($_SESSION['gift_vouchers'] as $voucher) {
					$voucher_amount = $voucher['amount'] * $_SESSION['nn_order']->info['currency_value'];
				}
		...
		$applepay_hidden_field = "<input type='hidden' value='".$input."' id='novalnet_applepay_data'>".
								"<input type='hidden' value='".htmlentities(json_encode($articleDetails))."' id='nn_apple_article_details'>";
