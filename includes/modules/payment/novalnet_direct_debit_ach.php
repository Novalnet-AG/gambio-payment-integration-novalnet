...
class novalnet_direct_debit_ach {
	 ...
	 */
		function selection() {
			...
			$selection = [
					'id'          => $this->code,
					'module'      => $this->title,
					'description' =>'<link rel="stylesheet" type="text/css" href="ext/novalnet/css/novalnet.css">'.'<script type="text/javascript" src="' . DIR_WS_CATALOG . 'ext/novalnet/js/novalnet_sepa.js"></script>'.'<script type=text/javascript src="' . DIR_WS_CATALOG . 'ext/novalnet/js/novalnet.js"></script>'. NovalnetHelper::showPaymentDescription($this->code) . $this->info,
			];
	...
	function install() {
		...
		xtc_db_query("insert into `gx_configurations` (`key`, `value`, `sort_order`, `type`, `last_modified`) values ('configuration/MODULE_PAYMENT_NOVALNET_DIRECT_DEBIT_ACH_ORDER_STATUS', '2',  '6', 'order-status', now()) ");
		xtc_db_query("insert into `gx_configurations` (`key`, `value`, `sort_order`,  `last_modified`) values ('configuration/MODULE_PAYMENT_NOVALNET_DIRECT_DEBIT_ACH_SORT_ORDER', '4', '7',  now()) ");
		xtc_db_query("insert into `gx_configurations` (`key`, `value`, `sort_order`,  `last_modified`) values  ('configuration/MODULE_PAYMENT_NOVALNET_DIRECT_DEBIT_ACH_ENDCUSTOMER_INFO', '',  '8',  now())");
	}
