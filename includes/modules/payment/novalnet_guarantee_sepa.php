...
class novalnet_guarantee_sepa {
	...
	function __construct() {
		...
		$this->description  = ((defined('MODULE_PAYMENT_NOVALNET_GUARANTEE_SEPA_TEXT_TITLE')) ? MODULE_PAYMENT_NOVALNET_GUARANTEE_SEPA_TEXT_TITLE : '');
		...
		$this->sort_order 	= defined('MODULE_PAYMENT_NOVALNET_GUARANTEE_SEPA_SORT_ORDER') && MODULE_PAYMENT_NOVALNET_GUARANTEE_SEPA_SORT_ORDER != '' ? trim(MODULE_PAYMENT_NOVALNET_GUARANTEE_SEPA_SORT_ORDER) : 0;
	...
	function selection() {
		...
		$selection = [
			...
			'description' => '<link rel="stylesheet" type="text/css" href="ext/novalnet/css/novalnet.min.css">' . NovalnetHelper::showSepaDescription($this->code) . $this->info
				.'<script type="text/javascript" src="' . DIR_WS_CATALOG . 'ext/novalnet/js/novalnet_sepa.min.js"  integrity="sha384-0jd2jLGYkXr/YZXdZ2BKsmiq5yRxFfusF7mIpYmAptX8JLuG75eKGcbEeQRgLbhs"></script>'
	   ...
	   if(defined('MODULE_PAYMENT_NOVALNET_GUARANTEE_SEPA_FORCE') && MODULE_PAYMENT_NOVALNET_GUARANTEE_SEPA_FORCE != 'true'){
		$selection['fields'][] = ['title' => '<input type="hidden" name="nn_sepa_birthdate_error" id="nn_sepa_birthdate_error" value="'.MODULE_PAYMENT_NOVALNET_GUARANTEE_DOB_ERROR_MSG.'" onblur="return validateDateFormat(this)">'];
	   }
	   ...
	   if (!$is_b2b || (defined('MODULE_PAYMENT_NOVALNET_GUARANTEE_SEPA_FORCE') && MODULE_PAYMENT_NOVALNET_GUARANTEE_SEPA_FORCE == 'false')) {
		...
		$bic_field = ['title' => '<div id="novalnet_sepa_bic" style="display:none;" ><span id="iban">BIC</span>' . xtc_draw_input_field('novalnet_sepa_bic', '', 'id="novalnet_sepa_bic_field" autocomplete="off" placeholder="BIC" onkeypress="return NovalnetUtility.formatBic(event);" onchange="return NovalnetUtility.formatBic(event);"') . '</div>', 'field' => '',
		];
		...
		if (defined('MODULE_PAYMENT_NOVALNET_GUARANTEE_SEPA_TOKENIZATION') && MODULE_PAYMENT_NOVALNET_GUARANTEE_SEPA_TOKENIZATION == 'true') {
			if (!empty($sepa_saved_card_details) || !empty($_SESSION['sepa_saved_card_details'])) {
				...
				foreach ($sepa_saved_card_details as $key => $value) {
					...
					$oneclick  = (!empty($sepa_saved_card_details)) ? '<nobr class="sepa_saved_card_info"><input name = "novalnet_sepa_token" type="radio" class ="novalnet_sepa_saved_acc" value="'.$value['token'].'" '.$checked.' onclick/>'.sprintf(('IBAN %s'),$value['iban']) . '&nbsp;&nbsp;<a href="javascript:void(0);" id="'.$value['id'].'" class="token" >'.(defined('MODULE_PAYMENT_NOVALNET_REMOVE_DUPLICATE_TOKEN') ? MODULE_PAYMENT_NOVALNET_REMOVE_DUPLICATE_TOKEN : '').'</a></nobr>' : '';
					if(!empty($value['iban'])){
				...
				$selection['fields'][] = ['title' => '<span id="iban" style="display:none;">'.MODULE_PAYMENT_NOVALNET_SEPA_IBAN.'</span>'. xtc_draw_input_field('novalnet_sepa_iban', '', 'id="novalnet_sepa_iban_field" autocomplete="off" placeholder ="DE00 0000 0000 0000 0000 00" style="display:none; width:170%;" onkeypress="return NovalnetUtility.formatIban(event,\'novalnet_sepa_bic\');" onkeyup="return NovalnetUtility.formatIban(event,\'novalnet_sepa_bic\');" onchange="return NovalnetUtility.formatIban(event,\'novalnet_sepa_bic\');"'),
				];
			...
			$selection['fields'][] = [
				'title' => MODULE_PAYMENT_NOVALNET_SEPA_IBAN. xtc_draw_input_field('novalnet_sepa_iban', '', 'id="novalnet_sepa_iban_field" autocomplete="off" placeholder ="DE00 0000 0000 0000 0000 00" style="width:170%;"  onkeypress="return NovalnetUtility.formatIban(event,\'novalnet_sepa_bic\');" onkeyup="return NovalnetUtility.formatIban(event,\'novalnet_sepa_bic\');" onchange="return NovalnetUtility.formatIban(event,\'novalnet_sepa_bic\');"'), 'field' => '',
			];
			$selection['fields'][] = $bic_field;
			if(isset($dob_field)){
			$selection['fields'][] = $dob_field;
	...
	function pre_confirmation_check() {
		...
		if (!empty($_SESSION['novalnet_guarantee_sepa_token']) || !empty($_SESSION['novalnet_guarantee_sepa_create_token'])) {
			unset($_SESSION['novalnet_guarantee_sepa_token'], $_SESSION['novalnet_guarantee_sepa_create_token']);
		}
		if (!empty($_SESSION['novalnet_guarantee_sepa_birthdate']) || !empty($_SESSION['novalnet_sepa_iban'])) {
			unset($_SESSION['novalnet_guarantee_sepa_birthdate'], $_SESSION['novalnet_sepa_iban']);
		}
		...
		if (!empty($_REQUEST['novalnet_guarantee_sepabirthdate'])) {
			...
			&& (defined('MODULE_PAYMENT_NOVALNET_SEPA_STATUS') && MODULE_PAYMENT_NOVALNET_SEPA_STATUS == 'true')))) {
				...
				xtc_redirect(xtc_href_link(FILENAME_CHECKOUT_PAYMENT, 'payment_error=' . $this->code . '&error=' . $error_message .MODULE_PAYMENT_NOVALNET_VALID_ACCOUNT_CREDENTIALS_ERROR, 'SSL', true, false));
			}
			...
			xtc_redirect(xtc_href_link(FILENAME_CHECKOUT_PAYMENT, 'payment_error=' . $this->code . '&error=' . $_SESSION['error_message'], 'SSL', true, false));
		}
	}
	...
	function before_process() {
		global $order;
		if (!empty($_SESSION['error_message']) && (defined('MODULE_PAYMENT_NOVALNET_SEPA_STATUS') && MODULE_PAYMENT_NOVALNET_SEPA_STATUS == 'true')
		...
		if ((defined('MODULE_PAYMENT_'. strtoupper($this->code) .'_AUTHENTICATE') && constant('MODULE_PAYMENT_'. strtoupper($this->code) .'_AUTHENTICATE') == 'true' )&& (NovalnetHelper::getOrderAmount($_SESSION['novalnet']['payment_amount']) >= (defined('MODULE_PAYMENT_'. strtoupper($this->code) .'_MANUAL_CHECK_LIMIT') ? constant('MODULE_PAYMENT_' . strtoupper($this->code) . '_MANUAL_CHECK_LIMIT') : ''))) { // Authorize transaction
		...
		unset($_SESSION['novalnet_sepa_iban'], $_SESSION['novalnet']['payment_amount'], $_SESSION[$this->code .'_token'], $_SESSION[$this->code . '_create_token'], $_SESSION['novalnet_guarantee_sepa_birthdate'], $_SESSION['novalnet_sepa_bic'], $_SESSION['novalnet_sepa_new'], $_SESSION['sepa_saved_card_details']);
	}
	...
	function install() {
		...
		xtc_db_query("insert into `gx_configurations` (`key`, `value`, `sort_order`, `last_modified`) values ('configuration/MODULE_PAYMENT_NOVALNET_GUARANTEE_SEPA_SORT_ORDER', '2', '14', now()) ");
