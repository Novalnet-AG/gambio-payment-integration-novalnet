...
class novalnet_guarantee_invoice {
	...
	function selection() {
		...
		if (MODULE_PAYMENT_NOVALNET_GUARANTEE_INVOICE_STATUS == 'true') {
			if ($this->proceedToGuranteePayment()) {
				$selection = [
					'id'          => $this->code,
					'module'      => MODULE_PAYMENT_NOVALNET_GUARANTEE_INVOICE_TEXT_TITLE,
					'description' => NovalnetHelper::showPaymentDescription($this->code) . $this->info,
				];
				$is_b2b = (MODULE_PAYMENT_NOVALNET_GUARANTEE_INVOICE_ALLOW_B2B == 'true' && !empty($order->billing['company'])) ? true : false;
				if (!$is_b2b) { 
	...
	function pre_confirmation_check() {
		...
		if (!empty($_REQUEST['novalnet_guarantee_invoicebirthdate'])) {
			...
			if (!empty($error_message) && ((MODULE_PAYMENT_NOVALNET_GUARANTEE_INVOICE_FORCE == 'true'
			&& MODULE_PAYMENT_NOVALNET_INVOICE_STATUS == 'true' ))) {
	 ...
	 */
	 function before_process(){
		global $order;
		if (!empty($_SESSION['error_message']) && MODULE_PAYMENT_NOVALNET_INVOICE_STATUS == 'true' && MODULE_PAYMENT_NOVALNET_GUARANTEE_INVOICE_FORCE == 'true') {
		...
		|| MODULE_PAYMENT_NOVALNET_GUARANTEE_INVOICE_ALLOW_B2B == 'false')) {
		...
		if($this->code == 'novalnet_invoice'){
			$due_date_in_days = MODULE_PAYMENT_NOVALNET_INVOICE_DUE_DATE;
			...
			if (MODULE_PAYMENT_NOVALNET_INVOICE_AUTHENTICATE == 'true'&& (NovalnetHelper::getOrderAmount($order->info['total']) >= MODULE_PAYMENT_NOVALNET_INVOICE_MANUAL_CHECK_LIMIT)) { // Authorize transaction
			...
			if (MODULE_PAYMENT_NOVALNET_GUARANTEE_INVOICE_AUTHENTICATE == 'true'
			&& (NovalnetHelper::getOrderAmount($order->info['total']) >= MODULE_PAYMENT_NOVALNET_GUARANTEE_INVOICE_MANUAL_CHECK_LIMIT)) { // Authorize transaction
				$response = NovalnetHelper::sendRequest($params, NovalnetHelper::get_action_endpoint('authorize'));
			} else { // Captue transaction
				$response = NovalnetHelper::sendRequest($params, NovalnetHelper::get_action_endpoint('payment'));
			}
	...
	*/
	function proceedToGuranteePayment() {
		...
		if (MODULE_PAYMENT_NOVALNET_GUARANTEE_INVOICE_STATUS == 'true') {
			$country_check = NovalnetHelper::checkGuaranteeCountries(strtoupper($order->billing['country']['iso_code_2']),MODULE_PAYMENT_NOVALNET_GUARANTEE_INVOICE_ALLOW_B2B, $order->billing['company']);
