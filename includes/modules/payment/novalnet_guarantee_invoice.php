...
class novalnet_guarantee_invoice {
	...
	function __construct() {
		...
		$this->description  = ((defined('MODULE_PAYMENT_NOVALNET_GUARANTEE_INVOICE_TEXT_TITLE')) ? MODULE_PAYMENT_NOVALNET_GUARANTEE_INVOICE_TEXT_TITLE : '');
		...
		$this->sort_order 	= defined('MODULE_PAYMENT_NOVALNET_GUARANTEE_INVOICE_SORT_ORDER') && MODULE_PAYMENT_NOVALNET_GUARANTEE_INVOICE_SORT_ORDER != '' ? trim(MODULE_PAYMENT_NOVALNET_GUARANTEE_INVOICE_SORT_ORDER) : 0;
	 ...
	 */
	 function before_process(){
		...
		$due_date_in_days = defined('MODULE_PAYMENT_'. strtoupper($this->code) .'_DUE_DATE') ? 'MODULE_PAYMENT_ '. strtoupper($this->code) .' _DUE_DATE' : '';
		...
		unset($_SESSION['novalnet_guarantee_invoicebirthdate'], $_SESSION['novalnet']['payment_amount']);
	 }
	...
	function after_process() {
		...
		if (in_array($_SESSION['response']['transaction']['status'], array('CONFIRMED', 'ON_HOLD'))
		 || ($_SESSION['response']['transaction']['status'] == 'PENDING' && $_SESSION['response']['transaction']['payment_type'] == 'INVOICE')
			...
			$order->info['comments'] .= PHP_EOL. PHP_EOL.MODULE_PAYMENT_NOVALNET_PAYMENT_REFERENCE_TEXT .PHP_EOL;
			$order->info['comments'] .= sprintf(MODULE_PAYMENT_NOVALNET_PAYMENT_REFERENCE, 1, ($_SESSION['response']['transaction']['tid'])) . PHP_EOL;
	...
	function install() {
		...
		xtc_db_query("insert into `gx_configurations` (`key`, `value`, `sort_order`, `last_modified`) values ('configuration/MODULE_PAYMENT_NOVALNET_GUARANTEE_INVOICE_SORT_ORDER', '5', '13', now())");
	...
	*/
	function proceedToGuranteePayment() {
		...
		if (isset($order) && defined('MODULE_PAYMENT_NOVALNET_GUARANTEE_INVOICE_STATUS') && MODULE_PAYMENT_NOVALNET_GUARANTEE_INVOICE_STATUS == 'true') {
