...
class novalnet_sepa {
	...
	function selection() {
		...
		if (MODULE_PAYMENT_NOVALNET_SEPA_TOKENIZATION == 'true') {
			if (!empty($saved_card_details)) {
				...
				foreach ($saved_card_details as $key => $value) {
					...
					$oneclick  = (!empty($saved_card_details)) ? '<nobr><input name = "novalnet_sepa" type="radio" class ="novalnet_sepa_saved_acc" value="'.$value['token'].'" />'.sprintf(('IBAN %s'),$value['iban']) . '&nbsp;&nbsp;<a href="javascript:void(0);" id="'.$value['id'].'" class="token" >Delete</a></nobr>' : '';
					if(!empty($value['iban'])){
		...
		if (MODULE_PAYMENT_NOVALNET_SEPA_TOKENIZATION == 'true' && empty($saved_card_details)) {
			$selection['fields'][] = [
				'title' => MODULE_PAYMENT_NOVALNET_SEPA_IBAN. xtc_draw_input_field('novalnet_sepa_iban', '', 'id="novalnet_sepa_iban_field" autocomplete="off" placeholder ="DE00 0000 0000 0000 0000 00" onkeypress="return NovalnetUtility.formatIban(event,\'novalnet_sepa_bic\');" onkeyup="return NovalnetUtility.formatIban(event,\'novalnet_sepa_bic\');" onchange="return NovalnetUtility.formatIban(event,\'novalnet_sepa_bic\');"'),
			];
			$selection['fields'][] = $bic_field;
	...
	function pre_confirmation_check() {
		...
		if (!empty($_REQUEST['novalnet_sepa']) && $_REQUEST['novalnet_sepa'] != 'new' && MODULE_PAYMENT_NOVALNET_SEPA_TOKENIZATION == 'true') {
			...
			if (MODULE_PAYMENT_NOVALNET_SEPA_TOKENIZATION == 'true' && !empty($_REQUEST['nn_sepa_checkbox'])) {
	...
	function before_process() {
		...
		if (MODULE_PAYMENT_NOVALNET_SEPA_AUTHENTICATE == 'true'
		&& (NovalnetHelper::getOrderAmount($order->info['total']) >= MODULE_PAYMENT_NOVALNET_SEPA_MANUAL_CHECK_LIMIT)) { // Authorize transaction
	...
	function install() {
		...
		xtc_db_query("insert into `gx_configurations` (`key`, `value`, `sort_order`, `type`, `last_modified`) values ('configuration/MODULE_PAYMENT_NOVALNET_SEPA_AUTHENTICATE','false', '7' ,'switcher', now())");
