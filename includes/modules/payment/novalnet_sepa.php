...
class novalnet_sepa {
    ...
    function selection() {
			...
			$selection  = array(
					...
					'description' => MODULE_PAYMENT_NOVALNET_SEPA_DESC . '<br>' . $test_mode . '<br>' . $endcustomerinfo.'<br><script src="' . DIR_WS_CATALOG . 'includes/external/novalnet/js/novalnet_sepa.js' . '" type="text/javascript"></script><span style="color:red">'.$gurantee_error.'</span>');
			...
			if ($_SESSION['novalnet'][$this->code]['tid'] != '' && in_array($this->fraud_module,array('CALLBACK', 'SMS')) && $this->fraud_module_status && empty($_SESSION['novalnet'][$this->code]['guarantee'])) {
				...
				$data                                            = NovalnetHelper::getVendorDetails($this->code);
				...
				xtc_draw_hidden_field('', MODULE_PAYMENT_NOVALNET_VALID_ACCOUNT_CREDENTIALS_ERROR, 'id="nn_lang_valid_account_details"').
				xtc_draw_hidden_field('', MODULE_PAYMENT_NOVALNET_SEPA_SHOP_TYPE, 'id="nn_sepa_shopping_type"').'<div class="novalnet_loader" id="nn_loader" style="display:none"></div> <link rel="stylesheet" type="text/css" href="' . DIR_WS_CATALOG . 'includes/external/novalnet/css/novalnet.css' . '">';
				...
				$selection['fields'][] = array(
							...
							'field' => '<div class="nn_sepa_acc" style="display:block">' . xtc_draw_pull_down_menu($this->code . '_bank_country', NovalnetHelper::sepaBankCountry(), $order->billing['country']['iso_code_2'], 'id="' . $this->code . '_bank_country"') . '</div>'
				);
				$selection['fields'][] =  array(
				...
				if (!empty($_SESSION['novalnet'][$this->code]['guarantee'])) {
					...
					$selection['fields'][] = array(
						...
						<script type= text/javascript src="' . DIR_WS_CATALOG . 'includes/external/novalnet/js/datepicker-'.$_SESSION['language_code'].'.js"></script>
						<script type="text/javascript">
							var j = jQuery.noConflict();
							jQuery(document).ready(function() {
							j("#novalnet_sepa_dob").removeAttr("placeholder");
    ...
    function install() {
        ...
        ('MODULE_PAYMENT_NOVALNET_SEPA_GUARANTEE_MAXIMUM_ORDER_AMOUNT', '', '6', '19','',  '', now()),
    ...
    function keys() {
	    ...
	    $return_key = array_merge(array(
            ...
            'MODULE_PAYMENT_NOVALNET_SEPA_GUARANTEE_MAXIMUM_ORDER_AMOUNT',
	...
	function proceedToGuranteePayment($orderamount) {
		...
		$maximum_amount_gurantee    = trim(MODULE_PAYMENT_NOVALNET_SEPA_GUARANTEE_MAXIMUM_ORDER_AMOUNT) != '' ? trim(MODULE_PAYMENT_NOVALNET_SEPA_GUARANTEE_MAXIMUM_ORDER_AMOUNT) : '500000';
