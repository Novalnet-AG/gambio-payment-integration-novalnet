...
class novalnet_sepa {
	...
	function __construct() {
		...
		$this->sort_order 		= (defined('MODULE_PAYMENT_NOVALNET_SEPA_SORT_ORDER') && MODULE_PAYMENT_NOVALNET_SEPA_SORT_ORDER != '') ? trim(MODULE_PAYMENT_NOVALNET_SEPA_SORT_ORDER) : 0;
	...
	function selection() {
		...
		'id'          => $this->code,
		'module'      => $this->title,
		'description' => '<link rel="stylesheet" type="text/css" href="ext/novalnet/css/novalnet.min.css">'.
						'<script type="text/javascript" src="' . DIR_WS_CATALOG . 'ext/novalnet/js/novalnet_sepa.min.js"  integrity="sha384-0jd2jLGYkXr/YZXdZ2BKsmiq5yRxFfusF7mIpYmAptX8JLuG75eKGcbEeQRgLbhs"></script>' .NovalnetHelper::showSepaDescription($this->code) . $this->info.
		...
		if (defined('MODULE_PAYMENT_NOVALNET_SEPA_TOKENIZATION') && MODULE_PAYMENT_NOVALNET_SEPA_TOKENIZATION == 'true') {
			if (!empty($saved_card_details)) {
				...
				foreach ($saved_card_details as $key => $value) {
					...
					$oneclick  = (!empty($saved_card_details)) ? '<nobr class="sepa_saved_card_info"><input name = "novalnet_sepa" type="radio" class ="novalnet_sepa_saved_acc" value="'.$value['token'].'" '.$checked.' />'.sprintf(('IBAN %s'),$value['iban']) . '&nbsp;&nbsp;<a href="javascript:void(0);" id="'.$value['id'].'" class="token" >'.(defined('MODULE_PAYMENT_NOVALNET_REMOVE_DUPLICATE_TOKEN') ? MODULE_PAYMENT_NOVALNET_REMOVE_DUPLICATE_TOKEN : '').'</a></nobr>' : '';
					if(!empty($value['iban'])){
	...
	function install() {
		...
		xtc_db_query("insert into `gx_configurations` (`key`, `value`, `sort_order`,  `last_modified`) values ('configuration/MODULE_PAYMENT_NOVALNET_SEPA_SORT_ORDER', '3', '11',  now()) ");
