...
class novalnet_sepa {
    ...
    function selection() {
			...
			$test_mode       .=  ((MODULE_PAYMENT_NOVALNET_SEPA_SHOP_TYPE == 'ZEROAMOUNT' && MODULE_PAYMENT_NOVALNET_SEPA_GUARANTEE == 'False' ) || (MODULE_PAYMENT_NOVALNET_SEPA_SHOP_TYPE == 'ZEROAMOUNT' && MODULE_PAYMENT_NOVALNET_SEPA_GUARANTEE == 'True' &&  MODULE_PAYMENT_NOVALNET_SEPA_FORCE == 'True')) ? '<br>'.MODULE_PAYMENT_NOVALNET_ZERO_AMOUNT_TEXT : '';
			...
			if ($_SESSION['novalnet'][$this->code]['tid'] != '' && in_array($this->fraud_module,array('CALLBACK', 'SMS')) && $this->fraud_module_status && empty($_SESSION['novalnet'][$this->code]['guarantee'])) {
				...
				if (!empty($_SESSION['novalnet'][$this->code]['guarantee']) && $order->billing['company'] == '') {
    ...
    function pre_confirmation_check() {
		...
		if ($this->fraud_module_status && !empty($_SESSION['novalnet'][$this->code]['tid']) && $_SESSION['novalnet'][$this->code . '_newpin'] != 1 && empty($_SESSION['novalnet'][$this->code]['guarantee'])) {
			...
			if (MODULE_PAYMENT_NOVALNET_SEPA_SHOP_TYPE != 'ONECLICK' && ((!isset($post['novalnet_sepa_account_holder']) || trim($post['novalnet_sepa_account_holder']) == '') || (!isset($post['nn_sepa_iban']) || $post['nn_sepa_iban'] == ''))) {
				$payment_error_return = 'payment_error=' . $this->code . '&error_message=' .MODULE_PAYMENT_NOVALNET_VALID_ACCOUNT_CREDENTIALS_ERROR;
                xtc_redirect(xtc_href_link(FILENAME_CHECKOUT_PAYMENT, $payment_error_return, 'SSL', true, false));
            }else if(MODULE_PAYMENT_NOVALNET_SEPA_SHOP_TYPE == 'ONECLICK' && ((!isset($post['novalnet_sepa_account_holder']) || trim($post['novalnet_sepa_account_holder']) == '') || (!isset($post['nn_sepa_iban']) || $post['nn_sepa_iban'] == '')) && (!isset($post['novalnet_sepachange_account']) || $post['novalnet_sepachange_account'] == '1')){
				$payment_error_return = 'payment_error=' . $this->code . '&error_message=' .MODULE_PAYMENT_NOVALNET_VALID_ACCOUNT_CREDENTIALS_ERROR;
                xtc_redirect(xtc_href_link(FILENAME_CHECKOUT_PAYMENT, $payment_error_return, 'SSL', true, false));
		...
		$billing_address = array(
            ...
            if (!empty($_SESSION['novalnet'][$this->code]['guarantee']) && !empty($_SESSION['novalnet'][$this->code]['guarantee_error'])) {
				if ($delivery_address !== $billing_address) {
					...
					$error_message = MODULE_PAYMENT_NOVALNET_GUARANTEE_INVALID_AMOUNT;
				  }
			} elseif (!empty($_SESSION['novalnet'][$this->code]['guarantee']) && $order->billing['company'] == '') {
    ...
    function before_process() {
        ...
        if (!empty($_SESSION['novalnet'][$this->code]['tid']) && $_SESSION['novalnet'][$this->code . '_newpin'] != 1 && $this->fraud_module != 'False' && empty($_SESSION['novalnet'][$this->code]['guarantee'])) {
            ...
            if ((trim(MODULE_PAYMENT_NOVALNET_SEPA_MANUAL_CHECK_LIMIT) > 0 && $order_amount >= trim(MODULE_PAYMENT_NOVALNET_SEPA_MANUAL_CHECK_LIMIT) && MODULE_PAYMENT_NOVALNET_SEPA_AUTHENTICATE === 'authorize' ) || (empty (MODULE_PAYMENT_NOVALNET_SEPA_MANUAL_CHECK_LIMIT) && MODULE_PAYMENT_NOVALNET_SEPA_AUTHENTICATE == 'authorize' )){ // If condition true then order proceed on-hold transaction
		...
		if (MODULE_PAYMENT_NOVALNET_SEPA_GUARANTEE == 'True') {
			if ($orderamount >= $minimum_amount_gurantee  && in_array(strtoupper($order->billing['country']['iso_code_2']), array('DE', 'AT', 'CH')) && $order->info['currency'] == 'EUR' && $delivery_address === $billing_address) {
			  ...
			  if ($delivery_address !== $billing_address) {
					...
					return MODULE_PAYMENT_NOVALNET_GUARANTEE_INVALID_AMOUNT;
				  }
			}
		} else {
			if(isset($_SESSION['novalnet'][$this->code]['guarantee'])) {
