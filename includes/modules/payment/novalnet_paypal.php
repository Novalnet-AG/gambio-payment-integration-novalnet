...
class novalnet_paypal {
    ...
    public function __construct() {
        ...
        if (strpos(MODULE_PAYMENT_INSTALLED, $this->code) !== false) {
            ...
            $this->enabled   = ((defined('MODULE_PAYMENT_NOVALNET_PAYPAL_ENABLE_MODULE') && MODULE_PAYMENT_NOVALNET_PAYPAL_ENABLE_MODULE == 'True') ? true : false);
    ...
    function selection() {
        ...
        if ( MODULE_PAYMENT_NOVALNET_PAYPAL_SHOP_TYPE != 'ONECLICK' || (MODULE_PAYMENT_NOVALNET_PAYPAL_SHOP_TYPE == 'ONECLICK' && empty($payment_details))) { // Displaying iframe form type
            $selection  = array(
                'id'          => $this->code,
                'module'      => $title,
                'description' => '<div id="nn_normal_description">'.$this->description.'<br></div><div>' . $test_mode . '<br>' . $endcustomerinfo.'</div><script src="' . DIR_WS_CATALOG . 'ext/novalnet/js/novalnet_paypal.js' . '" type="text/javascript"></script>'.NovalnetHelper::enableJavascript(),
			);
			if($shop_version > '3810') {
    ...
    function payment_action(){
        ...
        if (isset($_SESSION['novalnet'][$this->code]['payment_amount']) && $_SESSION['novalnet'][$this->code]['novalnet_paypal_change_account'] != '1') { // Generate the redirect payments params
            ...
            if(empty($urlparam['payment_ref']) && MODULE_PAYMENT_NOVALNET_PAYPAL_SHOP_TYPE == 'ONECLICK'){
				unset($_SESSION['novalnet'][$this->code]['reference_transaction']);
			}
     ...
     */
     function getOnholdValue($urlparam) {
		 if (trim(MODULE_PAYMENT_NOVALNET_PAYPAL_MANUAL_CHECK_LIMIT) > 0 && $urlparam['amount'] >= trim(MODULE_PAYMENT_NOVALNET_PAYPAL_MANUAL_CHECK_LIMIT)) { // If condition true then order proceed on-hold transaction
			    return true;
            }
	 }
}
