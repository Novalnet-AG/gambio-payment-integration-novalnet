...
class novalnet_googlepay {
	...
	function doPaymentCall($order_no = '') {
		...
		if (empty($_SESSION['received_data'])) {
			...
			$params['transaction']['payment_type'] = 'GOOGLEPAY';
			$params['transaction']['currency'] = $order->info['currency'];
			$params['transaction']['test_mode'] = (MODULE_PAYMENT_NOVALNET_GOOGLEPAY_TEST_MODE == 'true') ? '1' : '0';
		...
		if (MODULE_PAYMENT_NOVALNET_GOOGLEPAY_AUTHENTICATE == 'true' && (NovalnetHelper::getOrderAmount($order->info['total']) >= MODULE_PAYMENT_NOVALNET_GOOGLEPAY_MANUAL_CHECK_LIMIT)) { // for on-hold transaction
	...
	function getParam() {
		...
		$data = array(
			 ...
			 'test_mode' 				=> (MODULE_PAYMENT_NOVALNET_GOOGLEPAY_TEST_MODE == 'true') ? '1' : '0',
			 ...
			 'seller_name' 				=> (MODULE_PAYMENT_NOVALNET_GOOGLEPAY_BUSINESS_NAME != '') ? MODULE_PAYMENT_NOVALNET_GOOGLEPAY_BUSINESS_NAME : STORE_NAME,
			 ...
			 'environment'				=> (MODULE_PAYMENT_NOVALNET_GOOGLEPAY_TEST_MODE == 'true') ? 'SANDBOX' : 'PRODUCTION',
			 'enforce_3d'				=> (MODULE_PAYMENT_NOVALNET_GOOGLEPAY_ENFORCE_3D_SECCURE == 'true') ? true : false,
