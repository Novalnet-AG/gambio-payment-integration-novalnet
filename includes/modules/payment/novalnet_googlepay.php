...
class novalnet_googlepay {
	var $code, $title, $enabled, $sort_order, $test_mode, $description, $info, $tmpStatus;
	...
	function before_process() {
		...
		$post = $_REQUEST;
		$session_order_id = !empty($_SESSION['nn_tempOID']) ? $_SESSION['nn_tempOID'] : (isset($_SESSION['tmp_oID']) ? $_SESSION['tmp_oID'] : null);
		if (isset($post['tid'])) {
			...
			if (!empty($_SESSION['nn_wallet_doredirect']) && $_SESSION['nn_wallet_doredirect'] == 'true') {
				...
				if ($response['result']['status'] == 'SUCCESS') {
					...
					xtc_redirect(xtc_href_link(FILENAME_CHECKOUT_PAYMENT, 'payment_error=' . $this->code . '&error=' . $error, 'SSL', true, false));
				}
			}
		}
		...
		if(empty($order) && !empty($session_order_id)) {
			...
			$order->info['comments'] .= NovalnetHelper::updateTransactionDetails($this->code,  $_SESSION['response'] ?? null);
		}else{
			$order->info['comments'] .= NovalnetHelper::updateTransactionDetails($this->code,  $_SESSION['response'] ?? null);
		}
	...
	function doPaymentCall($order_no = '') {
		...
		if (isset($data['result']) && $data['result']['status'] != 'SUCCESS' ) {
	...
	function install() {
		...
		xtc_db_query("insert into `gx_configurations` (`key`, `value`, `sort_order`, `last_modified`) values ('configuration/MODULE_PAYMENT_NOVALNET_GOOGLEPAY_BUTTON_HEIGHT', '40', '13',  now()) ");
	...
	function googlepayCart_page_button() {
		...
		&& (!isset($_SESSION['allow_checkout']) || $_SESSION['allow_checkout'] == 'true') && (strpos(MODULE_PAYMENT_INSTALLED, 'novalnet_config') == true && MODULE_PAYMENT_NOVALNET_SIGNATURE !='' && MODULE_PAYMENT_NOVALNET_PAYMENT_ACCESS_KEY !='' && MODULE_PAYMENT_NOVALNET_TARIFF_ID !=''))
		{
			...
			if (!empty($order->info['deduction']) && $_SESSION['customers_status']['customers_status_show_price_tax'] == '0') {
				$_SESSION['novalnet']['deduction_amount'] = isset($order->info['deduction']) ? $order->info['deduction'] : 0;
				$_SESSION['novalnet']['payment_amount'] = round($order->info['total'], 2) + round($order->info['tax'], 2);
			} else if (isset($order->info['deduction'])){
				$_SESSION['novalnet']['deduction_amount'] = $order->info['deduction'];
				$_SESSION['novalnet']['payment_amount'] = $order->info['total'];
			}
			include_once(DIR_FS_CATALOG .'lang/'. $_SESSION['language'].'/modules/payment/novalnet_googlepay.php');
			echo '<script src="' . DIR_WS_CATALOG . 'ext/novalnet/js/novalnet_wallet.min.js" type="text/javascript" integrity="sha384-2bN8cXWW1oVWnhst5+reVjZusq5hesa2kxWr6XYYEyJpXSWLswWgK9qxC6ZuwQzA"></script>'.xtc_draw_hidden_field('nn_googlepay_total_label', (defined('MODULE_PAYMENT_NOVALNET_GOOGLEPAY_TOTAL_LABEL') ? MODULE_PAYMENT_NOVALNET_GOOGLEPAY_TOTAL_LABEL : ''), 'id="nn_googlepay_total_label"').$this->getParam($order).'<input type="hidden" id="novalnet_googlepay_enable" value="'.$this->enabled.'"/><script src="https://ajax.googleapis.com/ajax/libs/jquery/3.5.1/jquery.min.js"></script><script src="https://cdn.novalnet.de/js/v2/NovalnetUtility.js"></script><script src="https://cdn.novalnet.de/js/v3/payment.js"></script>';
		}
	}
	...
	function getParam($nn_order = '') {
		...
		$product_name = isset($order->products[0]['name']) ? $order->products[0]['name'] : '';
		$data = array(
			 ...
			 'googlepay_button_height' 	=> !empty(MODULE_PAYMENT_NOVALNET_GOOGLEPAY_BUTTON_HEIGHT) ? MODULE_PAYMENT_NOVALNET_GOOGLEPAY_BUTTON_HEIGHT : '40',
		...
		if (!empty($GLOBALS['product']->data['products_id']) && in_array('product page', $display_page)) {
			...
			if (!empty($_SESSION['novalnet']['payment_amount']) && $_SESSION['customers_status']['customers_status_show_price_tax'] == '0') {
				...
				$data['total_amount'] = !empty($nn_order) ? (string)($nn_order->info['total'] * 100) : (string)($order->info['total'] * 100);
			}
		}
		...
		if (empty($nn_order->products) && !empty($order->products)) {
			$tax_amount = $order->info['tax'];
			if (!empty($_SESSION['nn_order']) && !empty($_SESSION['nn_order']->info['deduction'])) {
				$discount_tax = $xtPrice->xtcGetTax((float) $_SESSION['nn_order']->info['deduction'], $order->products[0]['tax']);
				$tax_amount = $tax_amount - $discount_tax;
			} elseif (!empty($_SESSION['gift_vouchers'])) {
				foreach ($_SESSION['gift_vouchers'] as $voucher) {
					$voucher_amount = $voucher['amount'] * $_SESSION['nn_order']->info['currency_value'];
				}
				$voucher_tax = $xtPrice->xtcGetTax((float) $voucher_amount, $order->products[0]['tax']);
				$tax_amount = $tax_amount - $voucher_tax;
			}
		...
		if(empty($articleDetails) && !empty($GLOBALS['product']->data['products_id'])) {
			...
			$articleDetails[] = array('label'=> $GLOBALS['product']->data['products_name'] . ' x ' .$qty,
									  'amount' => (string)($products_price['plain'] * 100) * $qty,
									  'type' => 'SUBTOTAL',
								);
			$tax_rate = $xtPrice->getTaxRateByTaxClassId($GLOBALS['product']->data['products_tax_class_id']);
			$tax = $xtPrice->xtcGetTax((float) $products_price['plain'], $tax_rate);
			$tax_amount = !empty($tax) ? $tax : $order->info['tax'];
			
			if (!empty($_SESSION['nn_order']) && !empty($_SESSION['nn_order']->info['deduction'])) {
				$discount_tax = $xtPrice->xtcGetTax((float) $_SESSION['nn_order']->info['deduction'], $tax_rate);
				$tax_amount = $tax_amount - $discount_tax;
			} elseif (!empty($_SESSION['gift_vouchers'])) {
				foreach ($_SESSION['gift_vouchers'] as $voucher) {
					$voucher_amount = $voucher['amount'] * $_SESSION['nn_order']->info['currency_value'];
				}
				$voucher_tax = $xtPrice->xtcGetTax((float) $voucher_amount, $tax_rate);
				$tax_amount = $tax_amount - $voucher_tax;
			}
			...
			$discount_price = isset($discount_price) ? $discount_price : 0;
			$data['total_amount'] = (string)(($products_price['plain'] * 100) - round($discount_price, 2) *100);
		}

		if ($current_page[0] == 'checkout_payment' || !empty($nn_order)) {
			if (!empty($nn_order->info['shipping_class']))
			{
				$articleDetails[] = array('label'=>  (defined('MODULE_PAYMENT_NOVALNET_GOOGLEPAY_SHIPPING_LABEL') ? MODULE_PAYMENT_NOVALNET_GOOGLEPAY_SHIPPING_LABEL : ''), 'amount' => (string)(round(($nn_order->info['shipping_cost'] * $nn_order->info['currency_value']), 2) * 100), 'type' => 'SUBTOTAL');
			}
			...
			if(!empty($nn_order->info['deduction']) || ($_SESSION['customers_status']['customers_status_discount'] != '0')) {		// To add discount
				...
				$articleDetails[] = array('label'=> (defined('MODULE_PAYMENT_NOVALNET_GOOGLEPAY_DISCOUNT_AND_GIFT_VOUCHER_LABEL') ? MODULE_PAYMENT_NOVALNET_GOOGLEPAY_DISCOUNT_AND_GIFT_VOUCHER_LABEL : ''), 'amount' => (string) (round($deduction, 2) * 100), 'type' => 'SUBTOTAL');
			}
			...
			if(!empty($_SESSION['gift_vouchers']) ) {
				...
				$articleDetails[] = array(
					'label' => (defined('MODULE_PAYMENT_NOVALNET_GIFT_VOUCHER_LABEL') ? MODULE_PAYMENT_NOVALNET_GIFT_VOUCHER_LABEL : ''),
			...
			if ($_SESSION['customers_status']['customers_status_show_price_tax'] != 0) {
				$data['total_amount'] = (string)round($nn_order->info['total'] * 100);
		...
		if ($current_page[0] == 'product_info') {
			if (!empty($_SESSION['nn_order']) && !empty($_SESSION['nn_order']->info['deduction'])) {
				$data['total_amount'] = (string)($data['total_amount'] - (round($_SESSION['nn_order']->info['deduction'], 2) * 100));
				$articleDetails[] = array('label'=> (defined('MODULE_PAYMENT_NOVALNET_GOOGLEPAY_DISCOUNT_AND_GIFT_VOUCHER_LABEL') ? MODULE_PAYMENT_NOVALNET_GOOGLEPAY_DISCOUNT_AND_GIFT_VOUCHER_LABEL : ''), 'amount' => (string) (round($_SESSION['nn_order']->info['deduction'], 2) * 100), 'type' => 'SUBTOTAL');
			} elseif (!empty($_SESSION['gift_vouchers'])) {
				foreach ($_SESSION['gift_vouchers'] as $voucher) {
					$voucher_amount = $voucher['amount'] * $_SESSION['nn_order']->info['currency_value'];
				}
				$data['total_amount'] = (string)($data['total_amount'] - (round($voucher_amount, 2) * 100));
				$articleDetails[] = array(
					'label' => (defined('MODULE_PAYMENT_NOVALNET_GIFT_VOUCHER_LABEL') ? MODULE_PAYMENT_NOVALNET_GIFT_VOUCHER_LABEL : ''),
		...
		if ($current_page[0] != 'checkout_payment') {
			if ($_SESSION['customers_status']['customers_status_show_price_tax'] != 0) {
				$data['orig_amount'] = (string) $data['total_amount'];
			} else {
		...
		$googlepay_hidden_field = "<input type='hidden' value='".$input."' id='novalnet_googlepay_data'>".
								"<input type='hidden' value='".htmlentities(json_encode($articleDetails))."' id='nn_google_article_details'>";
