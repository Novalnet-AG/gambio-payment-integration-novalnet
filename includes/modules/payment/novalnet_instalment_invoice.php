...
class novalnet_instalment_invoice {
	...
	function selection() {
		...
		$selection = [
			...
			'description' => NovalnetHelper::showPaymentDescription($this->code) . $this->info
										 ...
										 . '<script type=text/javascript src="' . DIR_WS_CATALOG . 'ext/novalnet/js/novalnet_instalment_invoice.js"></script>'
										 . '<input type="hidden" name="nn_instalmentinvoice_birthdate_error" id="nn_instalmentinvoice_birthdate_error" value="'.MODULE_PAYMENT_NOVALNET_GUARANTEE_DOB_ERROR_MSG.'">',
		];
	...
	function pre_confirmation_check() {
		...
		&& MODULE_PAYMENT_NOVALNET_INSTALMENT_INVOICE_ALLOW_B2B == 'false')) {
	...
	function before_process() {
		...
		if (MODULE_PAYMENT_NOVALNET_INSTALMENT_INVOICE_AUTHENTICATE == 'true'
		&& (NovalnetHelper::getOrderAmount($order->info['total']) >= MODULE_PAYMENT_NOVALNET_INSTALMENT_INVOICE_MANUAL_CHECK_LIMIT)) {
