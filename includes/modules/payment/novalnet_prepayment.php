...
class novalnet_prepayment {
    ...
    public function __construct() {
        ...
        $this->title        = defined('MODULE_PAYMENT_NOVALNET_PREPAYMENT_TEXT_TITLE') ? 'Novalnet '. MODULE_PAYMENT_NOVALNET_PREPAYMENT_TEXT_TITLE : '';
        $this->public_title =  defined('MODULE_PAYMENT_NOVALNET_PREPAYMENT_PUBLIC_TITLE') ? MODULE_PAYMENT_NOVALNET_PREPAYMENT_PUBLIC_TITLE : '';
        $this->sort_order   = 0;
        if (strpos(MODULE_PAYMENT_INSTALLED, $this->code) !== false) {
            ...
            $this->test_mode = ((defined('MODULE_PAYMENT_NOVALNET_PREPAYMENT_TEST_MODE') && MODULE_PAYMENT_NOVALNET_PREPAYMENT_TEST_MODE == 'true') ? true : false);// To check the test mode
    ...
    function selection() {
        ...
        $endcustomerinfo           = trim(strip_tags(MODULE_PAYMENT_NOVALNET_PREPAYMENT_ENDCUSTOMER_INFO)) != '' ? trim(strip_tags(MODULE_PAYMENT_NOVALNET_PREPAYMENT_ENDCUSTOMER_INFO)) : '';
		global $gx_version;
        ...
        $selection  = array(
                'id'		  => $this->code,
                'module' 	  => $title,
                'description' => MODULE_PAYMENT_NOVALNET_INV_PRE_DESC . '<br>' . $test_mode . '<br><span id ="notification_buyer_wrap">' . $endcustomerinfo.'</span><br>');
		if($shop_version > '3810') {
			if((!defined('MODULE_PAYMENT_NOVALNET_PAYMENT_LOGO_DISPLAY')) || MODULE_PAYMENT_NOVALNET_PAYMENT_LOGO_DISPLAY == 'true'){
				$selection['logo_url'] = xtc_href_link('images/icons/payment/novalnet_prepayment.png', '', 'SSL', false, false, false, true, true);
			}else {
				$selection['logo_url'] = '';
				$selection['logo_alt'] = '';
			}
    ...
    function before_process() {
        ...
        $urlparam['payment_type'] = 'INVOICE_START';
           // To send the prepayment due date to novalnet server
            if (!empty(trim(MODULE_PAYMENT_NOVALNET_PREPAYMENT_DUE_DATE)) && preg_match('/^\d+$/',trim(MODULE_PAYMENT_NOVALNET_PREPAYMENT_DUE_DATE))) {
                $urlparam['due_date'] = date("Y-m-d", strtotime(trim(MODULE_PAYMENT_NOVALNET_PREPAYMENT_DUE_DATE) . ' days'));
            }
    ...
    function check() {
        ...
        if($_SESSION['GX'] == '1'){
			  $check_query  = xtc_db_query("select `value` from `gx_configurations` where `key` = 'configuration/MODULE_PAYMENT_NOVALNET_PREPAYMENT_ALLOWED'");
    ...
    function install() {
		if($_SESSION['GX'] == '1'){
			...
			xtc_db_query("insert into `gx_configurations` ( `key`, `value`,   `sort_order`,  `last_modified`) values ('configuration/MODULE_PAYMENT_NOVALNET_PREPAYMENT_ALLOWED', '',  '1',   now()) ");
			xtc_db_query("insert into `gx_configurations` ( `key`, `value`,   `sort_order`,  `last_modified`) values ('configuration/MODULE_PAYMENT_NOVALNET_PREPAYMENT_ALIAS', 'NN_PREPAY',  '0',   now()) ");
			xtc_db_query("insert into `gx_configurations` ( `key`, `value`,   `sort_order`, `type`, `last_modified`) values  ('configuration/MODULE_PAYMENT_NOVALNET_PREPAYMENT_STATUS','false',  '2', 'switcher',now())");
			xtc_db_query("insert into `gx_configurations` ( `key`, `value`,   `sort_order`, `type`, `last_modified`) values ('configuration/MODULE_PAYMENT_NOVALNET_PREPAYMENT_TEST_MODE','false',  '3', 'switcher', now()) ");
			xtc_db_query("insert into `gx_configurations` ( `key`, `value`,   `sort_order`,  `last_modified`) values ('configuration/MODULE_PAYMENT_NOVALNET_PREPAYMENT_VISIBILITY_BY_AMOUNT', '',  '4', now()) ");
			xtc_db_query("insert into `gx_configurations` ( `key`, `value`,   `sort_order`,  `last_modified`) values ('configuration/MODULE_PAYMENT_NOVALNET_PREPAYMENT_DUE_DATE', '', '', now())");
			xtc_db_query("insert into `gx_configurations` ( `key`, `value`,   `sort_order`,  `last_modified`) values ('configuration/MODULE_PAYMENT_NOVALNET_PREPAYMENT_ENDCUSTOMER_INFO', '',  '5', now()) ");
			xtc_db_query("insert into `gx_configurations` ( `key`, `value`,   `sort_order`,  `last_modified`) values ('configuration/MODULE_PAYMENT_NOVALNET_PREPAYMENT_SORT_ORDER', '0',  '6',  now()) ");
			xtc_db_query("insert into `gx_configurations` ( `key`, `value`,   `sort_order`, `type`, `last_modified`) values ('configuration/MODULE_PAYMENT_NOVALNET_PREPAYMENT_ORDER_STATUS', '0',  '7', 'order-status', now()) ");
			xtc_db_query("insert into `gx_configurations` ( `key`, `value`,   `sort_order`, `type`, `last_modified`) values ('configuration/MODULE_PAYMENT_NOVALNET_PREPAYMENT_CALLBACK_ORDER_STATUS', '0',  '8', 'order-status', now()) ");
			xtc_db_query("insert into `gx_configurations` ( `key`, `value`,   `sort_order`, `type`, `last_modified`) values ('configuration/MODULE_PAYMENT_NOVALNET_PREPAYMENT_PAYMENT_ZONE', '0', '9', 'geo-zone',now()) ");
      ...
      VALUES
        ('MODULE_PAYMENT_NOVALNET_PREPAYMENT_ALLOWED', '', '6', '1', '', '', now()),
        ('MODULE_PAYMENT_NOVALNET_PREPAYMENT_ALIAS', 'NN_PREPAY', '6', '0', '', '', now()),
        ('MODULE_PAYMENT_NOVALNET_PREPAYMENT_STATUS','false', '6', '2', 'xtc_mod_select_option(array(\'true\' => MODULE_PAYMENT_NOVALNET_TRUE,\'false\' => MODULE_PAYMENT_NOVALNET_FALSE),\'MODULE_PAYMENT_NOVALNET_PREPAYMENT_STATUS\'," .'MODULE_PAYMENT_NOVALNET_PREPAYMENT_STATUS'. ",' , '',now()),
        ('MODULE_PAYMENT_NOVALNET_PREPAYMENT_TEST_MODE','false', '6', '3', 'xtc_mod_select_option(array(\'true\' => MODULE_PAYMENT_NOVALNET_TRUE,\'false\' => MODULE_PAYMENT_NOVALNET_FALSE),\'MODULE_PAYMENT_NOVALNET_PREPAYMENT_TEST_MODE\'," .'MODULE_PAYMENT_NOVALNET_PREPAYMENT_TEST_MODE'. ",' , '', now()),
        ('MODULE_PAYMENT_NOVALNET_PREPAYMENT_VISIBILITY_BY_AMOUNT', '', '6', '4','',  '', now()),
        ('MODULE_PAYMENT_NOVALNET_PREPAYMENT_DUE_DATE', '', '6', '6','',  '', now()),
        ('MODULE_PAYMENT_NOVALNET_PREPAYMENT_ENDCUSTOMER_INFO', '', '6', '5','',  '', now()),
        ('MODULE_PAYMENT_NOVALNET_PREPAYMENT_SORT_ORDER', '0', '6', '6', '',  '', now()),
        ('MODULE_PAYMENT_NOVALNET_PREPAYMENT_ORDER_STATUS', '0',  '6', '7', 'xtc_cfg_pull_down_order_statuses(', 'xtc_get_order_status_name', now()),
        ('MODULE_PAYMENT_NOVALNET_PREPAYMENT_CALLBACK_ORDER_STATUS', '0',  '6', '8', 'xtc_cfg_pull_down_order_statuses(', 'xtc_get_order_status_name', now()),
    ...
    function remove() {
		if($_SESSION['GX'] == '1' ){
			xtc_db_query("delete from `gx_configurations` where `key` in ('" . implode("', '", array_merge($this->keys(),array('configuration/MODULE_PAYMENT_NOVALNET_PREPAYMENT_ALIAS'))) . "')");
		}else{
		   xtc_db_query("delete from " . TABLE_CONFIGURATION . " where configuration_key in ('" . implode("', '", array_merge($this->keys(),array('MODULE_PAYMENT_NOVALNET_PREPAYMENT_ALIAS'))) . "')");
	    }
    }
    ...
    function keys() {
		...
		if($_SERVER['REQUEST_URI'] === DIR_WS_CATALOG.'admin/modules.php?set=payment&module=novalnet_prepayment&action=edit'){
		  echo '<script src="' . DIR_WS_CATALOG . 'ext/novalnet/js/novalnet.js" type="text/javascript"></script>';
	    }
	    if($_SESSION['GX'] == '1' ){
			$alias_menu = array();
		   if($gx_version >= '3.1.1.0' ){
			    $alias_menu = array_merge($alias_menu,array('configuration/MODULE_PAYMENT_NOVALNET_PREPAYMENT_ALIAS'));
		    }
          ...
          $alice_val = 'configuration/MODULE_PAYMENT_NOVALNET_PREPAYMENT_ALIAS';
		}else{
			$alias_menu = array();
		   if($gx_version >= '3.1.1.0' ) {
			   $alias_menu = array_merge($alias_menu,array('MODULE_PAYMENT_NOVALNET_PREPAYMENT_ALIAS'));
		      }
		   $return_key = array_merge(array(
          ...
          $alice_val = 'MODULE_PAYMENT_NOVALNET_PREPAYMENT_ALIAS';
		}
