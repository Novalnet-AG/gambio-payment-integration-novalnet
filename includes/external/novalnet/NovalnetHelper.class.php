...
class NovalnetHelper {
    ...
    function getCommonParms($datas) {
        ...
        $urlparam              = array_merge(array(
            ...
            'system_name' 			=> 'Gambio',
            'system_version' 		=> $gx_version . '-NN-' . '11.1.3',
     ...
     */
     public static function getIpAddress($ipaddress) {
		 return (filter_var($ipaddress, FILTER_VALIDATE_IP, FILTER_FLAG_IPV6) || $ipaddress == '') ? '127.0.0.1' : $ipaddress;
	 }
    ...
    public static function checkPaymentStatus($payment_name, $response,$payment_param = '') {
        ...
        if ($response['status'] == 100 || ($payment_name == 'novalnet_paypal' && $response['status'] == 90)) { // Payment Success
            ...
            if (in_array($payment_name, array( 'novalnet_invoice', 'novalnet_prepayment' ))) {
                ...
                $cashpayment_slip_details = array_merge($nearest_store,array('due_date'       => $response['cashpayment_due_date']));
     ...
     */
     public static function logInitialTransaction($datas) {
        ...
        xtc_db_perform('novalnet_callback_history', array(
			 ...
			 'callback_amount'      =>(in_array($datas['payment_type'], array('novalnet_invoice', 'novalnet_prepayment','novalnet_cashpayment')) || in_array($_SESSION['novalnet'][$datas['payment_type']]['gateway_response']['tid_status'], array(85,86,90)) || (!empty($datas['tid_status']) && $datas['tid_status'] == 90) ) ? '0' : $amount,
			 'order_no'             =>  !empty($datas['new_order_no']) ? $datas['new_order_no'] : $datas['order_no'],
			), "insert");
    }
	...
	public static function transactionCommentsForm ($response,$paymentname) {
		...
		if(in_array($response['payment_id'],array('40','41'))){
			$trans_comments .= MODULE_PAYMENT_NOVALNET_MENTION_PAYMENT_CATEGORY;
		}
