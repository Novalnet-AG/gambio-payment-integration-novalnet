...
$request = $_POST;
	if (!empty($request['nn_refund_confirm']) && $request['refund_trans_amount'] != '') { // To process refund process
            ...
            if (isset($request['refund_paymenttype']) && $request['refund_paymenttype'] == 'SEPA') { // Param assigning for sepa payment only
                $refund_params['iban']           = $request['refund_payment_type_iban'];
                $refund_params['bic']            = $request['refund_payment_type_bic'];
                $refund_params['account_holder'] = $request['refund_payment_type_accountholder'];
            }
            ...
            if ($datas['payment_id'] == 27 && !empty($due_date)) { // Due_date added for payment invoice ,prepayment only
            ...
            if ($data['status'] == 100) { // Amount update success in shop
                $message                 = PHP_EOL . sprintf(utf8_encode(MODULE_PAYMENT_NOVALNET_TRANS_UPDATED_MESSAGE), xtc_format_price_order(($amount_change_request['amount'] / 100), 1, $datas['currency']), date(DATE_FORMAT, strtotime(date('Y-m-d'))), date('H:i:s')) . PHP_EOL;
                ...
                if ($datas['payment_id'] == 27) { // Allowed only payment id 27
