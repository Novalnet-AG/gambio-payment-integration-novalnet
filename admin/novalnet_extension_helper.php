...
if (isset($request['nn_manage_confirm']) && !empty($request['trans_status'])) {
	...
	if ($response['result']['status'] == 'SUCCESS') {
		...
		$messageStack->add_session($response['result']['status_text']);
	}
...
function updateOrderStatus($order_id, $order_status, $message) {
	...
	xtc_db_query("insert into " . TABLE_ORDERS_STATUS_HISTORY . " (orders_id, orders_status_id, date_added, customer_notified, comments, customer_id) values ('".xtc_db_input($order_id)."', '".xtc_db_input($order_status)."', '" . date('Y-m-d H:i:s') . "', '1', '".xtc_db_input($message)."', '0')");
}
?>
