...
class novalnet_vendor_script {
    ...
    protected $subscription_payment = array('SUBSCRIPTION_STOP','SUBSCRIPTION_REACTIVATE');
    ...
    protected $payment_groups = array(
        ...
        'novalnet_sepa'       => array('DIRECT_DEBIT_SEPA', 'REFUND_BY_BANK_TRANSFER_EU','RETURN_DEBIT_SEPA', 'SUBSCRIPTION_STOP', 'SUBSCRIPTION_REACTIVATE' ,'DEBT_COLLECTION_SEPA', 'CREDIT_ENTRY_SEPA', 'GUARANTEED_DIRECT_DEBIT_SEPA','GUARANTEED_SEPA_BOOKBACK'),
        ...
        'novalnet_invoice'    => array('INVOICE_START', 'INVOICE_CREDIT', 'SUBSCRIPTION_STOP', 'SUBSCRIPTION_REACTIVATE', 'GUARANTEED_INVOICE','GUARANTEED_INVOICE_BOOKBACK','REFUND_BY_BANK_TRANSFER_EU'),
    ...
    function __construct($capture_parameter) {
        ...
        } else if (in_array($capture_parameter['payment_type'], array_merge($this->chargebacks_payments, $this->collection_payments))) { // Add tid_payment parameter as mandatory for collection/chargeback payments
        ...
        } else if (($this->callback_params['payment_type'] == 'SUBSCRIPTION_STOP') || (isset($this->callback_params['subs_billing']) && $this->callback_params['subs_billing'] == 1 && $this->callback_params['status'] != 100)) { // Cancel the subscription process
            $this->cancel_subscription();
        } else if ($this->callback_params['payment_type'] == 'SUBSCRIPTION_REACTIVATE') { // Reactivate the subscription process
            $this->subscription_reactivate();
        } else if ($this->callback_params['payment_type'] == 'TRANSACTION_CANCELLATION' && in_array($this->order_reference['current_order_previous_status'], array('75','91','99'))) {
    ...
    function validate_capture_params($capture_parameter) {
        ...
        if (!empty($capture_parameter['vendor_activation'])) {
            ...
            if (!in_array($capture_parameter['payment_type'], array_merge($this->payments, $this->chargebacks_payments, $this->collection_payments, $this->subscription_payment)) && $capture_parameter['payment_type'] != 'TRANSACTION_CANCELLATION') { // Validate payment type
            ...
            } elseif ( isset( $capture_parameter ['subs_billing'] ) && $capture_parameter ['subs_billing'] == '1' || in_array($capture_parameter['payment_type'], $this->subscription_payment)) {
                $capture_parameter['shop_tid'] = $capture_parameter['signup_tid'];
            }
        }
    ...
    function zero_level_process() {
        ...
        if (isset($this->callback_params['subs_billing']) && $this->callback_params['subs_billing'] == 1 && $this->callback_params['status'] == 100 && in_array($this->callback_params['tid_status'], array( 85, 91,90, 98, 99, 100))) {
            $callback_comments_recurring = PHP_EOL.sprintf(MODULE_PAYMENT_NOVALNET_CALLBACK_RECURRING_COMMENTS, $this->callback_params['shop_tid'], $this->formatted_amount, date("Y-m-d", strtotime(date("Y-m-d")) . ' days'), date("H:i:s"), $this->callback_params['tid']);
            ...
            $createDate = new DateTime($charging_date);
            $charging_date = $createDate->format('d.m.Y');
            $callback_comments .= PHP_EOL.sprintf(MODULE_PAYMENT_NOVALNET_CALLBACK_CHARGING_DATE_COMMENTS, $charging_date). PHP_EOL . sprintf(MODULE_PAYMENT_NOVALNET_CALLBACK_REFERENCE_TID_COMMENTS, $this->callback_params['shop_tid']). PHP_EOL;
            $this->create_new_order($this->order_reference['order_no'], $callback_comments, (array) $this->transaction_details,$callback_comments_recurring);
            ...
            $this->update_callback_comments(array(
                ...
                if(isset($this->callback_params['subs_id']) && $this->callback_params['subs_id'] !='' ){
                    $next_charge_date = ($this->callback_params['next_subs_cycle']) ? $this->callback_params['next_subs_cycle'] : $this->callback_params['paid_until'];
                    $createDate = new DateTime($next_charge_date);
                    $next_charge_date = $createDate->format('d.m.Y');
                    $next_charge_transactionCommentsForm = PHP_EOL.sprintf(MODULE_PAYMENT_NOVALNET_CALLBACK_CHARGING_DATE_COMMENTS, $next_charge_date).PHP_EOL;
                }
                if(in_array($this->callback_params['payment_type'],array('GUARANTEED_INVOICE','GUARANTEED_DIRECT_DEBIT_SEPA'))){
                    if(in_array($this->callback_params['payment_type'],array('GUARANTEED_INVOICE')) && $this->callback_params['tid_status'] == '100'  ){
						$transaction_comments .= MODULE_PAYMENT_NOVALNET_MENTION_PAYMENT_CATEGORY.PHP_EOL;
                ...
                if( in_array($this->callback_params['tid_status'],array(99,91)) && $this->order_reference['current_order_previous_status'] == 75){
                    $order_status = ($this->callback_params['payment_type'] == 'GUARANTEED_INVOICE') ? constant('MODULE_PAYMENT_NOVALNET_INVOICE_CALLBACK_ORDER_STATUS') : constant('MODULE_PAYMENT_'.strtoupper($this->order_reference['payment_type']).'_ORDER_STATUS');
                    $callback_comments .= PHP_EOL. sprintf(MODULE_PAYMENT_GUARANTEE_PAYMENT_PENDING_TO_HOLD_MESSAGE, $this->callback_params['shop_tid'],date(DATE_FORMAT, strtotime(date('d.m.Y'))), date('H:i:s'));
                } else if($this->callback_params['tid_status'] == 100 && in_array( $this->order_reference['current_order_previous_status'], array(75,91,99))){
                    if((($this->callback_params['payment_type'] == 'GUARANTEED_INVOICE') &&  $this->order_reference['current_order_previous_status'] == 75)){
                    ...
                    if(in_array($this->callback_params['payment_type'],array('GUARANTEED_INVOICE','INVOICE_START')) && in_array($this->order_reference['current_order_previous_status'], array(75, 91))){
                        ...
                        NovalnetHelper::guarantee_mail(array(
                            'comments' => '<br>' . $nn_comments.PHP_EOL.$transactionCommentsForm.PHP_EOL. $callback_comments,
                            'order_no' => $this->order_reference['order_no'],
                        ));
                    }
                }
                ...
                xtc_db_perform(TABLE_ORDERS_STATUS_HISTORY, array(
                    ...
                    'customer_notified'=> 1,
                    'comments'         => $nn_comments. $transactionCommentsForm.$callback_comments 
                ));
                ...
                $test_mode_text     = ($this->callback_params['test_mode'] == 1) ? MODULE_PAYMENT_NOVALNET_TEST_ORDER_MESSAGE : '';
                $callback_comments .= MODULE_PAYMENT_NOVALNET_TRANSACTION_DETAILS . PHP_EOL . MODULE_PAYMENT_NOVALNET_TRANSACTION_ID.$this->callback_params['shop_tid'].PHP_EOL.$test_mode_text.PHP_EOL;
                $server_error = NovalnetHelper::getServerResponse($this->callback_params);
   ...
   function handle_communication_failure($order_table_details){
        if (!empty($order_table_details['payment_method']) && strpos($order_table_details['payment_method'], 'novalnet') !== false) {
            ...
            $transaction_comments = PHP_EOL . MODULE_PAYMENT_NOVALNET_TRANSACTION_DETAILS . PHP_EOL . MODULE_PAYMENT_NOVALNET_TRANSACTION_ID . $this->callback_params['tid'] . ((int)(!empty($this->callback_params['test_mode']) || constant('MODULE_PAYMENT_' . strtoupper($order_table_details['payment_method']) . '_TEST_MODE') == 'True') ? PHP_EOL . MODULE_PAYMENT_NOVALNET_TEST_ORDER_MESSAGE . PHP_EOL : '');
            if(isset($this->callback_params['subs_id']) && $this->callback_params['subs_id'] !='' ){
                $next_charge_date = ($this->callback_params['next_subs_cycle']) ? $this->callback_params['next_subs_cycle'] : $this->callback_params['paid_until'];
                $createDate = new DateTime($next_charge_date);
                $next_charge_date = $createDate->format('d.m.Y');
                $transaction_comments .= PHP_EOL.sprintf(MODULE_PAYMENT_NOVALNET_CALLBACK_CHARGING_DATE_COMMENTS, $next_charge_date).PHP_EOL;
            }
			...
			'subs_id'               => !empty($this->callback_params['subs_id']) ? $this->callback_params['subs_id'] : '',
		...
		xtc_db_perform('novalnet_callback_history', array(
			...
			if($this->callback_params['subs_id'] != ''){
				xtc_db_perform('novalnet_subscription_detail', array(
					'order_no'           => $this->callback_params['order_no'],
					'subs_id'            => $this->callback_params['subs_id'],
					'tid'                => $this->callback_params['shop_tid'],
            ...
            if(!empty($this->callback_params['tid_status']) && in_array($this->callback_params['tid_status'], array('100','99','98','91','90','85','86'))) {
                ...
                if(($this->callback_params['payment_type'] == 'PAYPAL' && $this->callback_params['tid_status'] == '90') || ($this->callback_params['payment_type'] == 'PRZELEWY24' && $this->callback_params['tid_status'] == '86')) {
                ...
                $order_comments = MODULE_PAYMENT_NOVALNET_TRANSACTION_DETAILS . PHP_EOL . MODULE_PAYMENT_NOVALNET_TRANSACTION_ID.':'. $this->callback_params['shop_tid'].PHP_EOL.$test_order_message;
                $transaction_status_failed = NovalnetHelper::getServerResponse($this->callback_params);
                $cancel_order_comments = $order_comments.PHP_EOL.MODULE_PAYMENT_NOVALNET_CANCEL_ORDER_MESSAGE.$transaction_status_failed;
                xtc_db_perform(TABLE_ORDERS, array(
                    'orders_status' => '99',
     ...
     * @param string $callback_comments_recurring
     */
    function create_new_order($order_no, $callback_comments, $transaction_details,$callback_comments_recurring) {
        ...
        $customer_dbvalue = xtc_db_fetch_array(xtc_db_query("SELECT customers_email_address, customers_cid, customers_gender, customers_dob, customers_fax,customers_telephone FROM " . TABLE_CUSTOMERS . " WHERE customers_id=".$order_details['customers_id']));
        $addressbook_dbvalue = xtc_db_fetch_array(xtc_db_query("SELECT entry_firstname,entry_lastname,entry_gender, entry_company, entry_street_address,entry_house_number,entry_suburb,entry_city,entry_postcode,entry_state,entry_country_id FROM " . TABLE_ADDRESS_BOOK . " WHERE customers_id=".$order_details['customers_id']));
        $updated_country = xtc_db_fetch_array(xtc_db_query("select countries_name from " . TABLE_COUNTRIES . " where countries_id = '" . (int)$addressbook_dbvalue['entry_country_id'] . "'"));
        $update_column = array('firstname','lastname','gender','company','street_address','house_number','suburb','city','postcode','state');
        foreach($update_column as $update_value){
            $order_details['customers_'.$update_value] = $addressbook_dbvalue['entry_'.$update_value];
            $order_details['delivery_'.$update_value] = $addressbook_dbvalue['entry_'.$update_value];
            $order_details['billing_'.$update_value] = $addressbook_dbvalue['entry_'.$update_value];
        }
        $order_details['customers_country'] = $order_details['delivery_country'] =  $order_details['billing_country'] = $updated_country['countries_name'];
        $order_details['billing_name'] = $order_details['delivery_name'] = $order_details['customers_name'] = $addressbook_dbvalue['entry_firstname'].$addressbook_dbvalue['entry_lastname'];
        $order_details['customers_telephone'] = $customer_dbvalue['customers_telephone'];
        $order_details['customers_email_address'] = $customer_dbvalue['customers_email_address'];
        ...
        $guarantee_order_comments = '';
        if(in_array($this->callback_params['payment_type'],array('GUARANTEED_INVOICE','GUARANTEED_DIRECT_DEBIT_SEPA'))){
            $guarantee_order_comments  .= MODULE_PAYMENT_NOVALNET_MENTION_PAYMENT_CATEGORY;
        }
        $test_mode               = !empty($this->callback_params['test_mode']) ? MODULE_PAYMENT_NOVALNET_TEST_ORDER_MESSAGE . PHP_EOL : '';
        $novalnet_order_comments = MODULE_PAYMENT_NOVALNET_TRANSACTION_DETAILS . PHP_EOL . MODULE_PAYMENT_NOVALNET_TRANSACTION_ID . ' ' . $this->callback_params['tid'] . PHP_EOL . $test_mode.$charging_date_comments.PHP_EOL;;
        list($order_comments, $bank_details) = (in_array($transaction_details['payment_type'], array('novalnet_invoice', 'novalnet_prepayment'))) ? $this->form_novalnet_comments($callback_comments) : array($novalnet_order_comments, '');
        ...
        if (!empty($order_comments)) {
            xtc_db_perform(TABLE_ORDERS, array(
                'orders_status' => $new_order_status,
                'comments'      => $guarantee_order_comments.$order_comments
        ...
        self::send_notify_mail(array(
                'comments' => '<br>' .$callback_comments_recurring. $callback_comments,
                'order_no' => $order_id,
           ),true,false);
    }
   ...
   function recurring_mail($datas,$guarantee_bankdetails=false){
    ...
    function cancel_subscription() {
        ...
        $subs_termination_reason = ($this->callback_params['termination_reason'] != '') ? $this->callback_params['termination_reason'] : NovalnetHelper::getServerResponse($this->callback_params);
        ...
        $row = xtc_db_fetch_array($db_query);
            xtc_db_perform(TABLE_ORDERS, array(
    ...
    /**
     * To process reactivate the subscription
     *
     */
    function subscription_reactivate(){
        // Get the subscription cancel message from server response
        $subs_termination_reason = '';
        // Update the subscription reason in novalnet table
        $this->update_subscription_reason(array(
            'termination_reason' => $subs_termination_reason,
            'termination_at'     => '',
        ...
        $callback_comments = PHP_EOL . sprintf(MODULE_PAYMENT_SUBSCRIPTION_REACTIVE_MESSAGE, $this->callback_params['shop_tid'], date("Y-m-d", strtotime(date("Y-m-d")) . ' days'),date("H:i:s"));
        $db_query = xtc_db_query('SELECT order_no from novalnet_subscription_detail WHERE tid = "' . $this->callback_params['shop_tid'] . '"');
        $row = xtc_db_fetch_array($db_query);
        xtc_db_perform(TABLE_ORDERS, array(
        ...
        xtc_db_perform(TABLE_ORDERS_STATUS_HISTORY, array(
            'orders_id'        => $row['order_no'],
            'orders_status_id' => constant('MODULE_PAYMENT_'.strtoupper($this->order_reference['payment_type']).'_ORDER_STATUS'),
            'date_added'       => date("Y-m-d H:i:s"),
            'customer_notified'=> 1,
            'comments'         => $callback_comments
        ));
     ...
     *
     * @param $charging_date_comments string
     * @return array
     */
    function form_novalnet_comments($charging_date_comments) {
        ...
        $novalnet_comments .= $charging_date_comments.PHP_EOL;
        $novalnet_comments .= MODULE_PAYMENT_NOVALNET_INVOICE_COMMENTS_PARAGRAPH . PHP_EOL;
        $novalnet_comments .= MODULE_PAYMENT_NOVALNET_DUE_DATE . date("d.m.Y", strtotime(!empty($this->callback_params['due_date']) ? $this->callback_params['due_date'] : '')) . PHP_EOL;
        $novalnet_comments .= MODULE_PAYMENT_NOVALNET_ACCOUNT_HOLDER . ': '.$this->callback_params['invoice_account_holder'] . PHP_EOL;
        ...
        $novalnet_comments .= MODULE_PAYMENT_NOVALNET_BANK . (!empty($this->callback_params['invoice_bankname']) ? trim($this->callback_params['invoice_bankname']) : ''). PHP_EOL;
        ...
        $bank_details = array(
            ...
            'account_holder' => $this->callback_params['invoice_account_holder'],
