...
class NovalnetWebhooks {
	...
	function getOrderDetails(){
        ...
        if  (empty($order_number) && !empty($this->event_data['transaction']['order_no'])) {
			$novalnet_order_details =  xtc_db_fetch_array(xtc_db_query("SELECT * FROM novalnet_transaction_detail WHERE order_no = '".$this->event_data['transaction']['order_no']."'"));
			$shop_order_no = xtc_db_fetch_array(xtc_db_query("SELECT * FROM ".TABLE_ORDERS." WHERE orders_id = '".$this->event_data['transaction']['order_no']."'"));
		    $order_number = isset($shop_order_no['orders_id']) ? $shop_order_no['orders_id'] : '';
	...
	function handleCommunicationFailure($order_details) {
		if ($this->event_data['result']['status'] == 'SUCCESS') {
			...
			$this->sendMailToCustomer(array(
				...
				'language' 	  => ($this->order_lang['languages_id'] == 1) ? 'english' : 'german',
			...
			NovalnetHelper::updateOrderStatus($order_details['orders_id'], $this->event_data['result']['status_text'], $this->event_data, $order_details['payment_method']);
	...
	function handleInstalment() {
		...
		&& in_array($this->event_data['transaction']['payment_type'], array('INSTALMENT_INVOICE','INSTALMENT_DIRECT_DEBIT_SEPA'))) {
			...
			$comment = PHP_EOL.sprintf(NOVALNET_WEBHOOK_NEW_INSTALMENT_NOTE, $this->parent_tid, $this->event_tid,xtc_format_price_order(($this->event_data['instalment']['cycle_amount']/100), 1, $this->event_data['transaction']['currency']), gmdate('d-m-Y')).' .';
			if($this->event_data['transaction']['payment_type'] == 'INSTALMENT_INVOICE'){
			   $comment .=PHP_EOL.NovalnetHelper::getBankDetails($this->event_data);
			   $comment .= PHP_EOL.NovalnetHelper::formInstalmentPaymentReference($this->event_data);
	...
	function handleInstalmentCancel() {
		...
		if ($this->event_data['transaction']['status'] == 'CONFIRMED') {
			$instalment_details = (!empty($this->order_details['nn_trans_details']['instalment_cycle_details'])) ? json_decode($this->order_details['nn_trans_details']['instalment_cycle_details'], true) : unserialize($this->order_details['nn_trans_details']['payment_details']);
            if (!empty($instalment_details)) {
			    if(isset($this->event_data['transaction']['refund'])){
				$comments = PHP_EOL. sprintf((MODULE_PAYMENT_NOVALNET_INSTALMENT_CANCEL_ALLCYCLES_TEXT_FIRST), $this->parent_tid, gmdate('d.m.Y'));
				if (isset($this->event_data['transaction']['refund']['amount']) && !empty($this->event_data['transaction']['refund']['amount'])) {	
	...
	function sendMailToCustomer($datas, $update_comment=false){
		...
		//Set lang issue in log (core file session set lang if lang not in session lot of log occurs in module)
