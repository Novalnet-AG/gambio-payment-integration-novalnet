...
class NovalnetWebhooks {
	...
	function getOrderDetails(){
		...
		$orderNumber = !empty($novalnet_order_details['order_no']) ? $novalnet_order_details['order_no'] : $this->event_data['transaction']['order_no'];
		if(empty($orderNumber)) {
			$this->displayMessage(array( 'message' => 'Order reference not found in the shop' ));
		}
		$shop_order_details = xtc_db_fetch_array(xtc_db_query("SELECT payment_method,payment_class,orders_id, customers_id,orders_status,comments,language FROM ".TABLE_ORDERS." WHERE orders_id = '".$orderNumber."'"));
	...
	function handleCommunicationFailure($order_details) {
		if ($this->event_data['result']['status'] == 'SUCCESS') {
			...
			if (in_array($order_details['payment_class'], array('novalnet_googlepay','novalnet_applepay'))) {
				$order->info['comments'] .= NovalnetHelper::updateTransactionDetails($order_details['payment_class'], $this->event_data);
			...
			$this->displayMessage(array( 'message' => 'Novalnet callback received' ));
			return;
		}
	}
	...
	function handleTransactionCapture() {
		if ($this->order_details['nn_trans_details']['status'] != $this->event_data['transaction']['status']) {
			...
			$comments = PHP_EOL.sprintf(MODULE_PAYMENT_NOVALNET_TRANS_CONFIRM_SUCCESSFUL_MESSAGE, gmdate('d-m-Y'), gmdate('H:i:s')).PHP_EOL.PHP_EOL;
			...
			if (in_array($this->order_details['shop_order_details']['payment_method'], array('novalnet_invoice', 'novalnet_guarantee_invoice', 'novalnet_instalment_invoice'))) {
				...
				if (in_array($this->order_details['shop_order_details']['payment_method'], array('novalnet_invoice', 'novalnet_guarantee_invoice'))) {
					$order_comment .= PHP_EOL. MODULE_PAYMENT_NOVALNET_PAYMENT_REFERENCE_TEXT .PHP_EOL;
					$order_comment .= sprintf(MODULE_PAYMENT_NOVALNET_PAYMENT_REFERENCE, 1, ('TID ' . $this->parent_tid)) . PHP_EOL;
					$order_comment .= sprintf(MODULE_PAYMENT_NOVALNET_PAYMENT_REFERENCE, 2, ('BNR-' . MODULE_PAYMENT_NOVALNET_PROJECT_ID . '-' . $this->event_data['transaction']['order_no'])) . PHP_EOL;
				}
			}
			if (in_array($this->order_details['shop_order_details']['payment_method'], array('novalnet_instalment_sepa', 'novalnet_instalment_invoice'))) {
	...
	function handleTransactionCredit() {
		...
		$order_status = '';
		$comments = sprintf(NOVALNET_WEBHOOK_CREDIT_NOTE, $this->parent_tid, xtc_format_price_order(($this->event_data['transaction']['amount']/100), 1, $this->event_data['transaction']['currency']), gmdate('d-m-Y H:i:s'), $this->event_tid);
		$order_status = NovalnetHelper::getOrderStatus($this->event_data['transaction']['status'], $this->order_details['shop_order_details']['payment_method']);
	...
	function handleChargeback() {
		if (($this->order_details['nn_trans_details']['status'] == 'CONFIRMED' || in_array($this->order_details['nn_trans_details']['status'], NovalnetHelper::$statuses['CONFIRMED'])) && !empty($this->event_data ['transaction'] ['amount'])) {
			$comments =sprintf(NOVALNET_WEBHOOK_CHARGEBACK_NOTE , $this->parent_tid, xtc_format_price_order(($this->event_data['transaction']['amount']/100), 1, $this->event_data['transaction']['currency']), gmdate('d.m.Y'), gmdate('H:i:s'), $this->event_tid);
	...
	function handleInstalment() {
		...
		&& in_array($this->event_data['transaction']['payment_type'], array('INSTALMENT_INVOICE','INSTALMENT_DIRECT_DEBIT_SEPA'))) {
			...
			$comment = sprintf(NOVALNET_WEBHOOK_NEW_INSTALMENT_NOTE, $this->parent_tid, xtc_format_price_order(($this->event_data['instalment']['cycle_amount']/100), 1, $this->event_data['transaction']['currency']), gmdate('d-m-Y'), $this->event_tid);
			...
			$transaction_comment = NovalnetHelper::insertTransactionDetails($this->order_details['shop_order'], $this->order_details['shop_order_details']['payment_method'], $this->event_data , false);
			$this->sendMailToCustomer(array(
			...
			$this->updateOrderStatusHistory($this->event_data['transaction']['order_no'], '', $transaction_comment, 1);
	...
	function handleInstalmentCancel() {
		...
		if ($this->event_data['transaction']['status'] == 'CONFIRMED') {
			...
			$comments .= sprintf(NOVALNET_WEBHOOK_INSTALMENT_CANCEL_NOTE, $this->parent_tid, date());
		}
		$this->updateNovalnetTransaction($novalnet_update_data, "tid='{$this->parent_tid}'");
		$this->updateOrderStatusHistory($this->event_data['transaction']['order_no'], '', $comments);
		$this->sendWebhookMail($comments);
				...
				|| in_array($this->order_details['nn_trans_details']['status'], NovalnetHelper::$statuses['ON_HOLD'])
					...
					if (in_array($this->event_data['transaction']['payment_type'], array( 'INSTALMENT_INVOICE','GUARANTEED_INVOICE', 'INVOICE', 'PREPAYMENT'))) {
						...
						if ($this->event_data['transaction']['payment_type'] != 'INSTALMENT_INVOICE') {
							$transaction_comments .= PHP_EOL. MODULE_PAYMENT_NOVALNET_PAYMENT_REFERENCE_TEXT .PHP_EOL;
							$transaction_comments .= sprintf(MODULE_PAYMENT_NOVALNET_PAYMENT_REFERENCE, 1, ('TID ' . $this->event_data['transaction']['tid'])) . PHP_EOL;
							if(!empty($this->order_details['shop_order']) || !empty($this->event_data['transaction']['invoice_ref'])) {
								$invoice_ref = (!empty($this->event_data['transaction']['invoice_ref'])) ? $this->event_data['transaction']['invoice_ref'] : 'BNR-' . MODULE_PAYMENT_NOVALNET_PROJECT_ID . '-' . $this->order_details['shop_order'];  
								$transaction_comments .= sprintf(MODULE_PAYMENT_NOVALNET_PAYMENT_REFERENCE, 2, $invoice_ref) . PHP_EOL;
							}
						}
					}
					if ('CASHPAYMENT' === $this->event_data ['transaction']['payment_type']) {
						...
						$transaction_comments .= NovalnetHelper::getBankDetails($response, $orderno = '');
					}
					if (in_array($this->event_data['transaction']['payment_type'], array( 'INSTALMENT_INVOICE','INSTALMENT_DIRECT_DEBIT_SEPA'))) {
					...
					if (! empty($this->event_data['transaction']['due_date'])) {
						$comments =sprintf(NOVALNET_WEBHOOK_TRANSACTION_UPDATE_NOTE_DUE_DATE, $this->event_tid, xtc_format_price_order(($amount/100), 1, $this->event_data['transaction']['currency']), $this->event_data['transaction']['due_date']);
					} else {
						$comments = sprintf(NOVALNET_WEBHOOK_TRANSACTION_UPDATE_NOTE, $this->event_tid, xtc_format_price_order(($amount/100), 1, $this->event_data['transaction']['currency']));
					}
				}
			}
			$this->updateNovalnetTransaction($novalnet_update_data, "tid='{$this->parent_tid}'");
	...
	function updateNovalnetTransaction($data, $parameters = '', $action = 'update') {
		...
		xtc_db_perform('novalnet_transaction_detail', $data, $action, $parameters);
	}
	...
	function sendWebhookMail($comments) {
		...
		// Send mail
		xtc_php_mail(EMAIL_FROM, STORE_NAME, $email_to, STORE_OWNER, '', '', '', '', '', $order_subject, $comments, '');
	}
	...
	function include_required_files($lang_data) {
		...
		foreach (glob(DIR_FS_CATALOG.'lang/'.$lang->language['directory'].'/modules/payment/novalnet*.php') as $filename) {
